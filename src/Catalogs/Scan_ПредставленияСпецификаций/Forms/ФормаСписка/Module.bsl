...
Процедура ОбновитьЭлементНаСервере() //rarus BProg_Dekin 14.03.2020 mantis 0014177 +-
	ИмяМетода 			= "GetListOfSpecificationView";
	СообщениеОбОшибке 	= "";
	Отказ 				= Ложь;
	...
	Если НЕ Отказ Тогда
		Scan_ВебСервисыРазборОтветов.РазборОтветаСправочникПредставленияСпецификаций(ТекстОтвета, Отказ, СообщениеОбОшибке, ИмяСобытияЖурналаРегистрации, ИмяМетода);
	КонецЕсли;
