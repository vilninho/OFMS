...
Функция ПроверитьВыражениеСервер(Выражение, Источник, ПроверятьНаличиеОперандов, СтрокаОшибки, КолонкаОшибки)
	...
	Если ОбъектДляПроверки=Неопределено Тогда
		ВывестиСообщениеПол(НСтр("ru='Не удалось создать экземпляр объекта-источника!'"));
	КонецЕсли;
	...
	Если Результат Тогда
		ВывестиСообщениеПол(НСтр("ru='Ошибок не обнаружено'"));
	Иначе
		ВывестиСообщениеПол(НСтр("ru='%1'"),,,,,ТекстОшибки)
	КонецЕсли;
...
Процедура СохранитьВыражение(Команда)
	...
	Иначе
		...
		//Если Вопрос("Выражение пусто. Продолжить?", РежимДиалогаВопрос.ДаНет) = КодВозвратаДиалога.Да Тогда
		//	ОповеститьОВыборе(СокрЛП(ТекстВыражения));
		//КонецЕсли;
