...
Процедура ОбновитьЭлементНаСервере() //rarus tenkam 11.09.2019 14442 ++
	Если ЗначениеЗаполнено(Объект.IDExternalSystem) Тогда
		...
		Если НЕ Отказ Тогда
			ТекСсылка = Scan_ВебСервисыРазборОтветов.РазборОтветаСправочникТипы(ТекЭлементОтвет,Отказ,СообщениеОбОшибке,ИмяСобытияЖурналаРегистрации,ИмяМетода,"Scan_ТипыКузова");
		КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка) //rarus tenkam 11.09.2019 14442 ++
	Если Объект.Ссылка = Справочники.Scan_ТипыКузова.ПустаяСсылка() Тогда
