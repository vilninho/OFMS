...
Процедура ПриЗаписи(Отказ)
	...
	Если Недействителен Тогда
		Scan_ПраваИНастройки.УдалитьПраваИНастройкиОбъекта(Ссылка);
	КонецЕсли;
	//rarus tenkam 06.10.2017 mantis 11198 ---
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ИндексРежимДоступаККомпаниям = ПроверяемыеРеквизиты.Найти("РежимДоступаККомпаниям");
	Если ПользовательСотрудник 
		И ИндексРежимДоступаККомпаниям <> Неопределено
		Тогда
		ПроверяемыеРеквизиты.Удалить(ИндексРежимДоступаККомпаниям);
	КонецЕсли;
	
	// Rarus tenkam 14.10.2021 mantis 17822 +++
	ИндексРеквизита = ПроверяемыеРеквизиты.Найти("ВидДилера");
	Если ПользовательСотрудник И ИндексРеквизита <> Неопределено Тогда
