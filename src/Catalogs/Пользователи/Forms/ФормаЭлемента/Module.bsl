...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//Если Объект.Ссылка.Пустая() Тогда
	//   	//Элементы.КодСотрудникаНаПортале.Видимость = Ложь;	 // rarus tenkam 23.12.2020 mantis 16906 +
	//	//Элементы.ЛогинНаПортале.Видимость = ложь; //rarus ozhnik 15888 29.06.2020 + 
	//	Элементы.ФормаСоздатьОбновитьПользователяПоКодуПортала.Заголовок = "Создать пользователя по коду портала";
	//	Элементы.ФормаСоздатьОбновитьПользователяПоКодуПортала.Видимость = Ложь;
	//	Элементы.КомпанииКонтрагентыПользователей.Видимость = Истина;
	...
	//	Объект.ПодразделениеОрганизации = Scan_ПраваИНастройки.Scan_Право("ПодразделениеДляВнешнихПользователей");
	//ИначеЕсли НЕ Объект.ПользовательПортала Тогда
	//	//Элементы.КодСотрудникаНаПортале.Видимость = Ложь;	// rarus tenkam 23.12.2020 mantis 16906 +
	//	//Элементы.ЛогинНаПортале.Видимость = ложь; //rarus ozhnik 15888 29.06.2020 + 
	//	Элементы.ФормаСоздатьОбновитьПользователяПоКодуПортала.Видимость = Ложь;
	//Иначе
	//	Элементы.ФормаСоздатьОбновитьПользователяПоКодуПортала.Заголовок = "Обновить пользователя с портала";
	//КонецЕсли;	
	////rarus vikhle 07.04.2020 mt 15695 ---
	//
	////rarus vikhle 20.04.2020 mt 15695 +++
	//Если Объект.ПользовательСотрудник Тогда
	//	Элементы.КомпанииКонтрагентыПользователей.Видимость =	Ложь;
	//Иначе
	//	ЗаполнитьКомпанииКонтрагентыПользователя();
	//	Элементы.Организация.Доступность = Ложь;
	//	Элементы.ПодразделениеОрганизации.Доступность = Ложь;
	//КонецЕсли;
	...
	Если Объект.Ссылка.Пустая() Тогда
		Объект.Организация = ПолучитьОрганизациюПоУмолчанию();
		Объект.ПодразделениеОрганизации = Scan_ПраваИНастройки.Scan_Право("ПодразделениеДляВнешнихПользователей");
		Объект.РежимДоступаККомпаниям = Перечисления.Scan_РежимДоступаПользователяККомпаниям.ГруппаКомпаний; //rarus kabany Дата: 08/07/2021 17897 +
		Объект.ВидДилера = ПредопределенноеЗначение("Перечисление.Scan_ВидыДилеров.Независимый"); // Rarus tenkam 13.10.2021 mantis 17822
		
	КонецЕсли;	
	
	Если НЕ Объект.Ссылка.Пустая() И НЕ Объект.ПользовательСотрудник Тогда //rarus vikhle 18.02.2021 добавил проверку на наличие ссылки
		ЗаполнитьКомпанииКонтрагентыПользователя();
	КонецЕсли;
	// rarus tenkam 23.12.2020 mantis 16906 ---
	
	ТребуетсяОбновлениеПоДаннымПортала = Истина;
	//rarus vikhle 20.04.2020 mt 15695 ---
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	//rarus FominskiyAS 26.02.2019  mantis 13863 +++
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Модифицированность И ТребуетсяОбновлениеПоДаннымПортала И Объект.ПользовательПортала
		И НЕ Объект.Недействителен И ЗначениеЗаполнено(Объект.КодСотрудникаНаПортале) Тогда
		Сообщить("Необходимо обновить пользователя по данным портала");
		Отказ = Истина;
	КонецЕсли;
	//rarus vikhle 07.05.2020 mt 15695 ---
	...
	Если Не КомпанииКонтрагентыПользователей.Количество() = 0 Тогда
		ТаблицаЗначений = ДанныеФормыВЗначение(КомпанииКонтрагентыПользователей,Тип("ТаблицаЗначений"));
		ТаблицаЗначений.Свернуть("Компания,Контрагент","Основной");
		ЗначениеВДанныеФормы(ТаблицаЗначений,КомпанииКонтрагентыПользователей);
		СтруктураОтбора = Новый Структура ("Компания",Справочники.Scan_Компании.ПустаяСсылка());
		ПустыеСтроки = КомпанииКонтрагентыПользователей.НайтиСтроки(СтруктураОтбора);
		Для Каждого Строка Из ПустыеСтроки Цикл
			КомпанииКонтрагентыПользователей.Удалить(Строка);	
		КонецЦикла;
						
		СтруктураОтбора = Новый Структура ("Основной", Истина);
		СтрокиСПризнакомОсновной = КомпанииКонтрагентыПользователей.НайтиСтроки(СтруктураОтбора);
		Если СтрокиСПризнакомОсновной.Количество() = 0 И КомпанииКонтрагентыПользователей.Количество() <> 0 Тогда
			КомпанииКонтрагентыПользователей[0].Основной = Истина;
			Сообщить (НСтр("ru = 'Был автоматически установлен признак Основной у строки №1'; en = 'The sign primary was automatically set for line No. 1'")); 
		ИначеЕсли СтрокиСПризнакомОсновной.Количество() > 1 Тогда
			Отказ = Истина;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.ПутьКДанным = "КомпанииКонтрагентыПользователей";
			Сообщение.Текст = НСтр("ru = 'У пользователя может быть только 1 основная компания!'; en = 'User can have only 1 primary company!'");
			Сообщение.Сообщить();
		КонецЕсли;	
	КонецЕсли;
	//rarus vikhle 21.04.2020 mt 15695 ---
	
	// Rarus tenkam 12.10.2021 mantis 17822 +++
	Если НЕ ТекущийОбъект.ПользовательСотрудник И ЗначениеЗаполнено(ТекущийОбъект.ВидДилера) Тогда
		// Проверим соответствие вида дилера пользователя виду дилера компании
		Для Каждого ТекСтрока Из КомпанииКонтрагентыПользователей Цикл
			ВидДилераКомпании = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекСтрока.Компания, "ВидДилера");
			Если ТекущийОбъект.ВидДилера <> ВидДилераКомпании Тогда
				Отказ = Истина;
				Сообщение = Новый СообщениеПользователю;
				Сообщение.ПутьКДанным = "ВидДилера";
				Сообщение.Текст = НСтр("ru = 'Вид дилера пользователя отличается от вида дилера компании!'");
				Сообщение.Сообщить();	
				Прервать;
			КонецЕсли;
		КонецЦикла;
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	//rarus vikhle 21.04.2020 mt 15695 +++
	РегистрыСведений.Scan_КомпанииПользователей.ЗаписьЗначенияРегистраСведений(ТекущийОбъект.Ссылка,КомпанииКонтрагентыПользователей.Выгрузить(,"Компания,Основной")); // rarus kabany 15.07.2021 17897 ++ // было Объект.Ссылка
	//rarus vikhle 21.04.2020 mt 15695	
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	//rarus vikhle 11.05.2020 mt 15695 +++	
	Если НЕ Объект.ПользовательСотрудник Тогда
		ПроверяемыеРеквизиты.Добавить("КомпанииКонтрагентыПользователей");
	КонецЕсли;
	//rarus vikhle 11.05.2020 mt 15695 ---
...
//rarus vikhle 07.04.2020 +++
&НаКлиенте
Процедура ПользовательПорталаПриИзменении(Элемент)
	// rarus tenkam 23.12.2020 mantis 16906 +++
		
	//Если Объект.ПользовательПортала И НЕ Объект.Ссылка.Пустая() Тогда
	//	// rarus tenkam 23.12.2020 mantis 16906 +++
	//	//Элементы.КодСотрудникаНаПортале.Видимость = Истина;
	//	Элементы.КодСотрудникаНаПортале.ТолькоПросмотр = Ложь;
	//	// rarus tenkam 23.12.2020 mantis 16906 ---		
	//	//Элементы.ЛогинНаПортале.Видимость = истина; //rarus ozhnik 15888 29.06.2020 + 
	//	Элементы.ФормаСоздатьОбновитьПользователяПоКодуПортала.Заголовок = "Обновить пользователя с портала";
	//	Элементы.ФормаСоздатьОбновитьПользователяПоКодуПортала.Доступность = Истина;
	//	Элементы.ФормаСоздатьОбновитьПользователяПоКодуПортала.Видимость = Истина;
	//	//Очистка пароля, чтобы нельзя было обойти запрет на смену пароля пользователям портала
	//	ПользовательИБПароль = "";
	//	УстановитьНаличиеПароля(ЭтотОбъект, Ложь, ПользователиКлиент.АвторизованныйПользователь());
	//ИначеЕсли Объект.ПользовательПортала И Объект.Ссылка.Пустая() Тогда
	//	// rarus tenkam 23.12.2020 mantis 16906 +++
	//	//Элементы.КодСотрудникаНаПортале.Доступность = Истина;
	//	//Элементы.КодСотрудникаНаПортале.Видимость = Истина;
	//	Элементы.КодСотрудникаНаПортале.ТолькоПросмотр = Ложь;
	//	// rarus tenkam 23.12.2020 mantis 16906 ---
	//	//Элементы.ЛогинНаПортале.Видимость = истина; //rarus ozhnik 15888 29.06.2020 + 
	//	Элементы.ФормаСоздатьОбновитьПользователяПоКодуПортала.Заголовок = "Создать пользователя с портала";
	//	Элементы.ФормаСоздатьОбновитьПользователяПоКодуПортала.Доступность = Истина;
	//	Элементы.ФормаСоздатьОбновитьПользователяПоКодуПортала.Видимость = Истина;
	//	//Очистка пароля, чтобы нельзя было обойти запрет на смену пароля пользователям портала
	//	ПользовательИБПароль = "";
	...
	//	Элементы.ФормаСоздатьОбновитьПользователяПоКодуПортала.Видимость = Ложь;
	//	// rarus tenkam 23.12.2020 mantis 16906 +++
	//	Элементы.КодСотрудникаНаПортале.ТолькоПросмотр = Истина;
	//	// rarus tenkam 23.12.2020 mantis 16906 ---
	//	//Элементы.ЛогинНаПортале.Видимость = ложь; //rarus ozhnik 15888 29.06.2020 + 
	//	Объект.КодСотрудникаНаПортале = 0;
	//КонецЕсли;	
		
	УправлениеДиалогомНаСервере();
	
	Если Объект.ПользовательПортала Тогда	
		// Очистка пароля, чтобы нельзя было обойти запрет на смену пароля пользователям портала
		ПользовательИБПароль = "";
		УстановитьНаличиеПароля(ЭтотОбъект, Ложь, ПользователиКлиент.АвторизованныйПользователь());
	КонецЕсли;                      	
...
//rarus vikhle 20.04.2020 +++
&НаКлиенте
Процедура ПользовательСотрудникПриИзменении(Элемент)
	// rarus tenkam 23.12.2020 mantis 16906 +++
	//Если Объект.ПользовательСотрудник Тогда
	//	КомпанииКонтрагентыПользователей.Очистить();
	//	Элементы.КомпанииКонтрагентыПользователей.Видимость = Ложь;
	//	Элементы.Организация.Доступность = Истина;
	//	Элементы.ПодразделениеОрганизации.Доступность = Истина;
	//	Объект.ПодразделениеОрганизации = ПредопределенноеЗначение("Справочник.Scan_Подразделения.ПустаяСсылка");
	//Иначе
	//	Элементы.КомпанииКонтрагентыПользователей.Видимость = Истина;
	//	Объект.Организация = ПолучитьОрганизациюПоУмолчанию();
	//	Объект.ПодразделениеОрганизации = Scan_ПраваИНастройки.Scan_Право("ПодразделениеДляВнешнихПользователей"); 
	//	Элементы.Организация.Доступность = Ложь;
	//	Элементы.ПодразделениеОрганизации.Доступность = Ложь;
	//	Сообщить("Были установлены организация и подразделение по умолчанию для внешних пользователей");
	//КонецЕсли;
	...
	Иначе
		...
		Объект.ПодразделениеОрганизации = Scan_ПраваИНастройки.Scan_Право("ПодразделениеДляВнешнихПользователей"); 
		Сообщить("Были установлены организация и подразделение по умолчанию для внешних пользователей");
		Объект.ВидДилера = ПредопределенноеЗначение("Перечисление.Scan_ВидыДилеров.ПустаяСсылка"); // rarus tenkam 13.10.2021 mantis 17822
...
Процедура СменитьПароль(Команда)
	...
	Если Объект.ПользовательПортала Тогда
		Сообщить("Для пользователей портала запрещено устанавливать пароль вручную");
		Возврат;
	КонецЕсли;	
	//rarus vikhle 28.04.2020 mt 15695---
	ДополнительныеПараметры = Новый Структура;
...
Процедура УстановитьУсловноеОформление()
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.КомпанииКонтрагентыПользователей.Имя);
	ПолеЭлемента.Использование = Истина;

	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("КомпанииКонтрагентыПользователей.Основной");
    ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Истина;
	...
	// rarus kabany 12.07.2021 17897 ++
	Элемент = УсловноеОформление.Элементы.Добавить();

	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.КомпанииКонтрагентыПользователейОсновной.Имя);
	ПолеЭлемента.Использование = Истина;

	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.РежимДоступаККомпаниям");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Перечисления.Scan_РежимДоступаПользователяККомпаниям.ГруппаКомпаний;
	ОтборЭлемента.Использование = Истина;
...
&НаСервере
Процедура УправлениеДиалогомНаСервере()	// rarus tenkam 23.12.2020 mantis 16906 +++
	
	Если Объект.ПользовательПортала Тогда	
		// Кнопка обновления с портала
		Элементы.ФормаСоздатьОбновитьПользователяПоКодуПортала.Видимость = Истина;
		Если НЕ Объект.Ссылка.Пустая() Тогда
			Элементы.ФормаСоздатьОбновитьПользователяПоКодуПортала.Заголовок = "Обновить пользователя с портала";
		Иначе 
	...
	Если Объект.ПользовательСотрудник Тогда
		...
		Элементы.Организация.Доступность = Истина;
		Элементы.ПодразделениеОрганизации.Доступность = Истина;
		Элементы.КонтактноеЛицоКомпании.Видимость = Ложь;
		// rarus agar 12.08.2021 18141 ++
		Элементы.РежимДоступаККомпаниям.Видимость = Ложь;
		// rarus agar 12.08.2021 18141 --
		Элементы.ВидДилера.Видимость = Ложь; // Rarus tenkam 13.10.2021 mantis 17822 +
	Иначе
		Элементы.КомпанииКонтрагентыПользователей.Видимость = Истина;
		Элементы.Организация.Доступность = Ложь;
		Элементы.ПодразделениеОрганизации.Доступность = Ложь;
		Элементы.КонтактноеЛицоКомпании.Видимость = Истина;
		// rarus agar 12.08.2021 18141 ++
		Элементы.РежимДоступаККомпаниям.Видимость = Истина;
		// rarus agar 12.08.2021 18141 --
		Элементы.ВидДилера.Видимость = Истина; // Rarus tenkam 13.10.2021 mantis 17822 +
	КонецЕсли;	
	
КонецПроцедуры	// rarus tenkam 23.12.2020 mantis 16906 ---	


//rarus vikhle 20.04.2020 mt 15695 +++

////////////////////////////////////////////////////////////////////////////////
// Управление компаниями контрагентами

//rarus vikhle 06.04.2021 mt 17484 +++
&НаКлиенте
Процедура КомпанииКонтрагентыПользователейКомпанияОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
...
Процедура КомпанияОбработкаВыбораПродолжение(Результат,ВыбраннаяКомпания) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		Элементы.КомпанииКонтрагентыПользователей.ТекущиеДанные.Компания = ВыбраннаяКомпания;
		КомпанииКонтрагентыПользователейКомпанияПриИзменении(Элементы.КомпанииКонтрагентыПользователейКомпания);	
	Иначе
		Элементы.КомпанииКонтрагентыПользователей.ТекущиеДанные.Компания = Неопределено;	
	КонецЕсли;
...
&НаКлиенте
Процедура КомпанииКонтрагентыПользователейКомпанияПриИзменении(Элемент)
	ТекущаяКомпания = Элементы.КомпанииКонтрагентыПользователей.ТекущиеДанные.Компания;
	Если ЗначениеЗаполнено(ТекущаяКомпания) Тогда
		Элементы.КомпанииКонтрагентыПользователей.ТекущиеДанные.Контрагент = ПолучитьДилера (ТекущаяКомпания);
	Иначе
		Элементы.КомпанииКонтрагентыПользователей.ТекущиеДанные.Контрагент = ПредопределенноеЗначение("Справочник.Scan_Контрагенты.ПустаяСсылка");
	КонецЕсли;
	
	ДобавитьЭлементКомпанииКонтрагентыПользователей(Элементы.КомпанииКонтрагентыПользователей.ТекущиеДанные);  //rarus kabany Дата: 06/07/2021 17897 +
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция ПолучитьДилера (КомпанияСсылка)
	Возврат РегистрыСведений.Scan_ВзаимосвязьКомпанийСКонтрагентами.ПолучитьДилераПоКомпании(КомпанияСсылка);		
КонецФункции
...
&НаСервере
Процедура ЗаполнитьКомпанииКонтрагентыПользователя()
	КомпанииКонтрагентыПользователей.Очистить();
	КомпанииКонтрагентыПользователей.Загрузить(РегистрыСведений.Scan_КомпанииПользователей.ПолучитьКомпаниюДилераПользователя(Объект.Ссылка));
	// rarus kabany 14.07.2021 17897 ++
	Если Объект.РежимДоступаККомпаниям = ПредопределенноеЗначение("Перечисление.Scan_РежимДоступаПользователяККомпаниям.ГруппаКомпаний") Тогда
		ОбновитьЛогикуКомпанииКонтрагентыПользователей(Ложь);
	КонецЕсли;
	// rarus kabany 14.07.2021 17897 --
КонецПроцедуры

&НаКлиенте
Процедура КомпанииКонтрагентыПользователейПриИзменении(Элемент)
	Модифицированность = Истина;
КонецПроцедуры

//rarus vikhle 20.04.2020 mt 15695 ---

//rarus kabany Дата: 06/07/2021 17897 ++
&НаКлиенте
Процедура РежимДоступаККомпаниямПриИзменении(Элемент)
	
	Если Объект.РежимДоступаККомпаниям = ПредопределенноеЗначение("Перечисление.Scan_РежимДоступаПользователяККомпаниям.ГруппаКомпаний") Тогда
		
		Если ЭтаФорма.КомпанииКонтрагентыПользователей.Количество() > 1  Тогда
			
			ПоказатьВопрос(Новый ОписаниеОповещения("ОтветНаВопросКомпанииКонтрагентыПользователей", ЭтотОбъект),
			Нстр("ru = 'Все строки, кроме строки с признаком ""Основной"" будет удалены из табличной части! Если строки с признаком ""Основной"" нет, будет отмечена первая строка!'; en = 'All rows, except for the row with the ""Main"" attribute, will be removed from the tabular section! If there is no line with ""Main"", the first line will be marked!'"), РежимДиалогаВопрос.ДаНет);
...
&НаКлиенте
Процедура ДобавитьЭлементКомпанииКонтрагентыПользователей(ТекущиеДанные)	
	Если Объект.РежимДоступаККомпаниям = ПредопределенноеЗначение("Перечисление.Scan_РежимДоступаПользователяККомпаниям.ГруппаКомпаний")
		И ЗначениеЗаполнено(ТекущиеДанные.Компания) Тогда
		ТекущиеДанные.Основной = Истина;		
	КонецЕсли;	
КонецПроцедуры

&НаКлиенте
Процедура ОтветНаВопросКомпанииКонтрагентыПользователей(Результат, Параметры) Экспорт
	
	Если Результат = КодВозвратаДиалога.Да Тогда
    	ОбновитьЛогикуКомпанииКонтрагентыПользователей(Истина);   
	Иначе
		Объект.РежимДоступаККомпаниям = ПредопределенноеЗначение("Перечисление.Scan_РежимДоступаПользователяККомпаниям.СписокКомпаний");	
	КонецЕсли;	
		
КонецПроцедуры

&НаСервере
Процедура ОбновитьЛогикуКомпанииКонтрагентыПользователей(ТребуетсяОповестетить = Ложь) 	
	...
	Если Не КомпанииКонтрагентыПользователей.Количество() = 0 Тогда
		...
		Если СтрокиСПризнакомОсновной.Количество() = 1 И КомпанииКонтрагентыПользователей.Количество() <> 0 Тогда		
			СтруктураОтбора = Новый Структура ("Основной",ЛОЖЬ);
			ПустыеСтроки = КомпанииКонтрагентыПользователей.НайтиСтроки(СтруктураОтбора);
			Для Каждого Строка Из ПустыеСтроки Цикл
				КомпанииКонтрагентыПользователей.Удалить(Строка);	
			КонецЦикла;
			Если ТребуетсяОповестетить Тогда
				ВывестиСообщениеПол(НСтр("ru = 'Строки без признака ""Основной"" удалены!'; en = 'Rows without the ""Primary"" tag have been deleted!'")); 
			КонецЕсли;
		ИначеЕсли СтрокиСПризнакомОсновной.Количество() = 0 и КомпанииКонтрагентыПользователей.Количество() <> 0  Тогда
			КомпанииКонтрагентыПользователей[0].Основной = Истина;
			СтруктураОтбора = Новый Структура ("Основной",ЛОЖЬ);
			ПустыеСтроки = КомпанииКонтрагентыПользователей.НайтиСтроки(СтруктураОтбора);
			Для Каждого Строка Из ПустыеСтроки Цикл
			...
			КонецЦикла;			
			Если ТребуетсяОповестетить Тогда
				ВывестиСообщениеПол(НСтр("ru = 'Был автоматически установлен признак Основной у строки №1. Строки без признака ""Основной"" удалены!'; en = 'The sign primary was automatically set for line No. 1. Rows without the ""Primary"" tag have been deleted!'"));		
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
//rarus kabany Дата: 07/07/2021 17897 ++

&НаКлиенте
Процедура КомпанииКонтрагентыПользователейПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	
	Если НЕ ЗначениеЗаполнено(Объект.РежимДоступаККомпаниям) Тогда
		Отказ = Истина;
		ВывестиСообщениеПол(Нстр("ru = 'Для начала заполните режим доступа к компаниям!'; en = 'First, fill out the access to companies mode!'"));
		Возврат;
	КонецЕсли;
	
	 //rarus kabany Дата: 13/07/2021 17897 ++
	  Если Объект.РежимДоступаККомпаниям = ПредопределенноеЗначение("Перечисление.Scan_РежимДоступаПользователяККомпаниям.ГруппаКомпаний")
		И КомпанииКонтрагентыПользователей.Количество() > 0 Тогда
         Отказ = Истина;
		 ВывестиСообщениеПол(Нстр("ru = 'В режиме доступа к компаниям ""Группа компаний"" можно добавить только одну компанию!'; en = 'Only one company can be added in the ""Company group"" access mode for companies!!'")); // rarus kabany 13.07.2021 17897 ++
	 КонецЕсли;
	 //rarus kabany Дата: 07/07/2021 17897 --
КонецПроцедуры

&НаКлиенте
Процедура КомпанииКонтрагентыПользователейКомпанияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Объект.РежимДоступаККомпаниям = ПредопределенноеЗначение("Перечисление.Scan_РежимДоступаПользователяККомпаниям.ГруппаКомпаний") Тогда
		...
		Отбор.ЛевоеЗначение    = Новый ПолеКомпоновкиДанных("ГруппыКомпаний");
		...
		ОткрытьФорму("Справочник.Scan_Компании.ФормаВыбора", ПараметрыФормы, Элементы.КомпанииКонтрагентыПользователейКомпания);
	КонецЕсли;

КонецПроцедуры

&НаКлиенте
Процедура КомпанииКонтрагентыПользователейКомпанияАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	Если Объект.РежимДоступаККомпаниям = ПредопределенноеЗначение("Перечисление.Scan_РежимДоступаПользователяККомпаниям.ГруппаКомпаний") Тогда
	СтандартнаяОбработка = Ложь;
	КонецЕсли;
КонецПроцедуры
...
&НаКлиенте
Процедура СоздатьОбновитьПользователяПоКодуПортала(Команда)
	Если Объект.Недействителен Тогда 
		Сообщение = Новый СообщениеПользователю();
    	Сообщение.Текст =  Нстр("ru = 'Пользователь недействителен!'; en = 'User is not valid!'");
    	Сообщение.Поле = "Объект.Недействителен";
		Сообщение.Сообщить();
	ИначеЕсли НЕ ЗначениеЗаполнено(Объект.КодСотрудникаНаПортале) Тогда 
		Сообщение = Новый СообщениеПользователю();
    	Сообщение.Текст =  Нстр("ru = 'Не заполнен код сотрудника на портале!'; en = 'The portal id is not filled!'");
    	Сообщение.Поле = "Объект.КодСотрудникаНаПортале";
		Сообщение.Сообщить();
	ИначеЕсли НЕ  Объект.ПользовательСотрудник И НЕ ЗначениеЗаполнено(КомпанииКонтрагентыПользователей) Тогда
		Сообщение = Новый СообщениеПользователю();
    	Сообщение.Текст =  Нстр("ru = 'Необходимо заполнить таблицу компаний и контрагентов внешнего пользователя'; en = 'Please fill in companies and dealer table'");
    	Сообщение.Поле = "КомпанииКонтрагентыПользователей";
		Сообщение.Сообщить();
	ИначеЕсли НЕ  ЗначениеЗаполнено(Объект.ПодразделениеОрганизации) Тогда
		Сообщение = Новый СообщениеПользователю();
    	Сообщение.Текст =  Нстр("ru = 'Не заполнено подразделение!'; en = 'The division is not filled!'");
    	Сообщение.Поле = "Объект.ПодразделениеОрганизации";
		Сообщение.Сообщить();	
	ИначеЕсли НЕ  ЗначениеЗаполнено(Объект.Организация) Тогда
		Сообщение = Новый СообщениеПользователю();
    	Сообщение.Текст =  Нстр("ru = 'Не заполнена организация!'; en = 'The organization is not filled!'");
    	Сообщение.Поле = "Объект.Организация";
		Сообщение.Сообщить();
	Иначе
		Если ЗначениеЗаполнено(Объект.Наименование) ИЛИ ЗначениеЗаполнено(ПользовательИБИмя) Тогда
			ПоказатьВопрос(Новый ОписаниеОповещения("ПослеЗакрытияВопроса", ЭтотОбъект),
         		                                       "Поля Полное имя и Имя для входа будут очищены, продолжить?", РежимДиалогаВопрос.ДаНет);
		Иначе
			СоздатьОбновитьПользователяПоКодуПорталаНаСервере();
		КонецЕсли;
	КонецЕсли;
...
&НаСервере
Процедура СоздатьОбновитьПользователяПоКодуПорталаНаСервере(Отказ = Ложь)
	//Проверка существующих пользователей с указанным кодом сотрудника на портале	
	Если Объект.Ссылка.Пустая() Тогда
		Запрос = Новый Запрос;
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |ГДЕ
		               |	Пользователи.КодСотрудникаНаПортале = &КодСотрудникаНаПортале";
		Запрос.УстановитьПараметр("КодСотрудникаНаПортале", Объект.КодСотрудникаНаПортале);
		Если НЕ Запрос.Выполнить().Пустой() Тогда
			Сообщить(Нстр("ru = 'В программе уже существует пользователь с указанным кодом на портале!'; en = 'The employee with the specified portal id already exists!'"));
			Возврат;
		КонецЕсли;		
	КонецЕсли;	
	...
	СтруктураПараметров.Вставить ("id",Объект.КодСотрудникаНаПортале);
	...
	Если Отказ Тогда
		Сообщить(Нстр("ru = 'Не удалось заполнить данные пользователя, подробности см. в журнале регистрации'; en = 'Failed to fill in user data,'"));
		Возврат;
	КонецЕсли;
			
	Если ЗначениеЗаполнено(ДанныеОСотрудникеСПортала.Фамилия) И ЗначениеЗаполнено(ДанныеОСотрудникеСПортала.Имя) Тогда 
		Объект.Наименование = ДанныеОСотрудникеСПортала.Фамилия + " " + ДанныеОСотрудникеСПортала.Имя;
	Иначе
		Объект.Наименование = ДанныеОСотрудникеСПортала.Логин;
	КонецЕсли;	
	
	Объект.ЛогинНаПортале = ДанныеОСотрудникеСПортала.Логин;	// rarus tenkam 23.12.2020 mantis 16906 +
	...
	Модифицированность = Истина;
	УстановитьНаличиеПароля(ЭтотОбъект, Истина, Пользователи.АвторизованныйПользователь());
	//Проверка возможности записи пользователя по переданным данным
	...
	Если Отказ Тогда
		Сообщить(Нстр("ru = 'Не удалось передать пароль пользователя на портал, подробности см. в журнале регистрации'; en = 'Failed to transfer user password to portal,'"));
		Возврат;
	КонецЕсли;

	Если НЕ ЗначениеЗаполнено(Объект.Сотрудник) Тогда
		Объект.Сотрудник = СоздатьОбновитьСотрудникаПоДаннымПортала(ДанныеОСотрудникеСПортала);
	Иначе 
		СоздатьОбновитьСотрудникаПоДаннымПортала(ДанныеОСотрудникеСПортала,Истина);	
	КонецЕсли;
    ТребуетсяОбновлениеПоДаннымПортала = Ложь;
	Записать();
	
	Если ЗначениеЗаполнено(Объект.Сотрудник) Тогда
		Справочники.Пользователи.ЗаписьЗначенияРегистраСведения(Объект.Сотрудник,Объект.Ссылка,Объект.ПодразделениеОрганизации,Объект.Организация);
	КонецЕсли;
	
	ТЗКонтактнойИнформации = ПолучитьТЗКонтактнойИнформации("Пользователь",ДанныеОСотрудникеСПортала);	
	УправлениеКонтактнойИнформацией.УстановитьКонтактнуюИнформациюОбъекта(Объект.Ссылка, ТЗКонтактнойИнформации,Ложь);
...
&НаСервере
Функция СоздатьОбновитьСотрудникаПоДаннымПортала(ДанныеОСотрудникеСПортала,ОбновитьСуществующего = Ложь)
	
	Если НЕ ОбновитьСуществующего Тогда
		ОбъектСотрудник = Справочники.Scan_Сотрудники.СоздатьЭлемент();
	Иначе
		ОбъектСотрудник = Объект.Сотрудник.ПолучитьОбъект();
	КонецЕсли;	
	
	// rarus agar 03.06.2021 16927 ++
	//ФИО = ДанныеОСотрудникеСПортала.Имя + " " 
	ФИО = ?(ЗначениеЗаполнено(ДанныеОСотрудникеСПортала.Имя),ДанныеОСотрудникеСПортала.Имя + " ","") + " " 
	// rarus agar 03.06.2021 16927 --
	+ ?(ЗначениеЗаполнено(ДанныеОСотрудникеСПортала.Отчество),ДанныеОСотрудникеСПортала.Отчество + " ","") 
	+  ДанныеОСотрудникеСПортала.Фамилия; 
	ОбъектСотрудник.Наименование =  ФИО;
	 
	ТЗКонтактнойИнформации = ПолучитьТЗКонтактнойИнформации("Сотрудник",ДанныеОСотрудникеСПортала);
	
	Попытка
		ОбъектСотрудник.Записать();
		УправлениеКонтактнойИнформацией.УстановитьКонтактнуюИнформациюОбъекта(ОбъектСотрудник.Ссылка,ТЗКонтактнойИнформации,Ложь);
		Возврат ОбъектСотрудник.Ссылка;
	Исключение
		Сообщить("Не удалось создать сотрудника " + ОписаниеОшибки());
