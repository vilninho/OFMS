...
Функция СформироватьДеревоОператоров()
	...
	Корень.Оператор  = "Функции произвольных условий";
	Корень.Текст     = "";
	...
	НоваяСтрока           = Корень.Строки.Добавить();
	НоваяСтрока.Оператор  = "Логические операторы и константы";
	НоваяСтрока.Текст     = "";
	НоваяСтрока.ЭтоГруппа = ИСТИНА;
	НоваяСтрока.Картинка  = 3;
	...
	НоваяСтрокаОперандов.Картинка  = -1;
	НоваяСтрокаОперандов           = НоваяСтрока.Строки.Добавить();
	НоваяСтрокаОперандов.Оператор  = "Целая часть";
...
//
&НаКлиенте
Процедура ОбработкаРезультатаСохранитьВыражение(РезультатОповещения, ДополнительныеПараметры=Неопределено) Экспорт
	
	Если РезультатОповещения = КодВозвратаДиалога.Да Тогда 
		ОповеститьОВыборе(СокрЛП(ДополнительныеПараметры));
	КонецЕсли;
	
КонецПроцедуры // ОбработкаРезультатаСохранитьВыражение()
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Источник") Тогда
		...
		Иначе
			...
			ДеревоПолей = Scan_ЗначимыеСобытия.ПолучитьДеревоМетаданныхОбъекта(Источник,"Объект");
	...
	Если РедактированиеШаблона Тогда
		...
		Заголовок = "Редактирование шаблона";
	Иначе
	...
	Выражение.УстановитьТекст(Справочники.Scan_ДействияНаЗначимыеСобытия.ОчиститьСодержимоеРедактора(Выражение.ПолучитьТекст()));
...
Процедура СохранитьВыражение(Команда)
	...
	Иначе
		...
		ОбработчикВопроса = Новый ОписаниеОповещения("ОбработкаРезультатаСохранитьВыражение", ЭтотОбъект,ТекстВыражения);
		
		// Формируем текст вопроса
		ТекстВопроса = НСтр("ru = 'Выражение пусто. Продолжить?'");
