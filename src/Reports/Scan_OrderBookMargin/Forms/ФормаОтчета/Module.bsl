...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОтчетМетаданные.имя = "Scan_OrderIntake" Тогда
		МассивКоманд = Новый Массив;
		КомандаСоздать = ЭтотОбъект.Команды.Добавить("Scan_ОтправитьПоШаблонуOI");
		КомандаСоздать.Действие  = "Scan_ОбщегоНазначенияКлиент.ОтправитьПоШаблонуOI";
		КомандаСоздать.Заголовок = НСтр("ru = 'Отправить по шаблону...'; en = 'Send in format...'");
		КомандаСоздать.Подсказка = НСтр("ru = 'Отправить отчет по настроенному шаблону Order Intake'; en = 'Send custom template report Order Intake'");
		МассивКоманд.Добавить(КомандаСоздать); 
		
		ОтчетыСервер.ВывестиКоманду(ЭтотОбъект, МассивКоманд, "ПодменюОтправить", Ложь, Ложь, "РассылкаОтчетов"); 	
...
Процедура ПриЗагрузкеПользовательскихНастроекНаСервере(НовыеПользовательскиеНастройкиКД)
	...
	ВыборкаПараметров = Отчет.КомпоновщикНастроек.Настройки.Отбор.Элементы;
	Для Каждого ТекЭлемент Из ВыборкаПараметров Цикл
		Если ТипЗнч(ТекЭлемент) = Тип("ЭлементОтбораКомпоновкиДанных") Тогда
			
			Если ТекЭлемент.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Продукт.ТипПродукта.ВидПродукта") Тогда
				ГрузовыеАвтомобили = Scan_ПраваИНастройки.Scan_Право("ВидПродуктаГрузовыеАвтомобили");	
				ТекЭлемент.ПравоеЗначение = ГрузовыеАвтомобили;
				
				//rarus vikhle 29.12.2021 m 18269 +++
				ПользовательскийОтбор = НовыеПользовательскиеНастройкиКД.Элементы.Найти(
				 ТекЭлемент.ИдентификаторПользовательскойНастройки);

				 
				ПользовательскийОтбор.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
				ПользовательскийОтбор.ПравоеЗначение = ГрузовыеАвтомобили;
...
Процедура ПослеФормирования(Результат, ПараметрыФормирования) Экспорт 
	...
	СброситьТекущуюОбласть();
	//rarus vikhle 14.01.2022 m 18269 +++
	//ПоказатьВыбранныйУровеньГруппировок();
	ПоказатьВыбранныйУровеньГруппировок(1);
	//rarus vikhle 14.01.2022 m 18269 ---
КонецПроцедуры
...
// rarus vikhle 22.12.2021 m 18269 +++
&НаКлиенте
Процедура ОкПоискПоКритериям(Команда)
	
	Если НЕ ЗначениеЗаполнено(КритерийПоиска) Тогда
		ВывестиСообщениеПол(Нстр("ru = 'Выберите критерий поиска!'")); //rarus vikhle 21.01.2022 АПК
		Возврат;
	КонецЕсли;
	
	Если СокрЛП(ПолеВвода) = "" Тогда
		ВывестиСообщениеПол(Нстр("ru = 'Список данных для поиска пустой!'")); //rarus vikhle 21.01.2022 АПК
		Возврат;
	КонецЕсли;
...
Процедура ПоискПоКритериямНаСервере()
	...
	Если КритерийПоиска = "НомерШасси" Тогда
		ЛевоеЗначение = "Шасси";
	ИначеЕсли КритерийПоиска = "НомерЗаказа" Тогда
		ЛевоеЗначение = "ЗаказНаЗавод.Наименование";
	ИначеЕсли КритерийПоиска = "Дилер" Тогда
		ЛевоеЗначение = "Дилер.Наименование";
		ЛевоеЗначениеДилера = "Дилер.КодДилераSOWA";
	ИначеЕсли КритерийПоиска = "НомерЗаявки" Тогда
		ЛевоеЗначение = "Заявка.НомерСоглашенияОПоставке";
	ИначеЕсли КритерийПоиска = "КонечныйКлиентКонтрагент" Тогда
		ЛевоеЗначение = "КонечныйКлиентКонтрагент.Наименование";
	ИначеЕсли КритерийПоиска = "КонечныйКлиентКомпания" Тогда
		ЛевоеЗначение = "КонечныйКлиентКомпания.Наименование";
	КонецЕсли;
