...
Процедура ПослеФормирования(Результат, ПараметрыФормирования) Экспорт 
	...
	ПоказатьВыбранныйУровеньГруппировок(1);//rarus vikhle 21.02.2022 m 18913
КонецПроцедуры
...
// rarus vikhle 18.02.2022 m 18913 +++
&НаКлиенте
...
Процедура ПоискПоКритериям(Команда)
	...
	Если НЕ ЗначениеЗаполнено(КритерийПоиска) Тогда
		ВывестиСообщениеПол(Нстр("ru = 'Выберите критерий поиска!'"));
		Возврат;
	КонецЕсли;
	
	Если СокрЛП(ПолеВвода) = "" Тогда
		ВывестиСообщениеПол(Нстр("ru = 'Список данных для поиска пустой!'"));
		Возврат;
	КонецЕсли;
...
Процедура ПоискПоКритериямНаСервере()
	...
	ИначеЕсли КритерийПоиска = "КонечныйКлиентКонтрагент" Тогда
		ЛевоеЗначение = "КлиентКонтрагентИзСУ.Наименование"; //rarus vikhle 22.02.2022 m 18913
	ИначеЕсли КритерийПоиска = "КонечныйКлиентКомпания" Тогда
		ЛевоеЗначение = "КлиентКомпанияИзСУ.Наименование"; //rarus vikhle 22.02.2022 m 18913
	ИначеЕсли КритерийПоиска = "НомерСУ" Тогда	
		ЛевоеЗначение = "СУ.Код";
	КонецЕсли;
