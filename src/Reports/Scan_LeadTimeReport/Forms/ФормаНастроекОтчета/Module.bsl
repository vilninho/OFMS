...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// rarus agar 04.08.2021 17758 ++
	СобытиеФормыДляКонтрольныхТочек = "ОткрытиеФормы";
	ЗаполнитьНастройкиПараметрами(НастройкиКД);
	// rarus agar 04.08.2021 17758 --
...
Процедура ЗаписатьИЗакрыть(Переформировать)
	...
	// rarus agar 10.06.2021 17758 ++
	ЗаполнитьПараметрыНастроек();
	// rarus agar 10.06.2021 17758 --
...
&НаСервере
Процедура ОбновитьКонтрольныеТочки()
	
	ПолучитьИспользуемыеКонтрольныеТочки();
	ОбновитьЭлементыТаблицыКонтрольныхТочек();
	
КонецПроцедуры

&НаСервере
Процедура ПолучитьИспользуемыеКонтрольныеТочки()
	
	ИспользуемыеКонтрольныеТочки.Очистить();
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("П1", ВидLeadTime);
	Запрос.Текст = "ВЫБРАТЬ
	|	Scan_КонтрольныеТочкиLeadTimeВидыLeadTime.Ссылка КАК КонтрольнаяТочкаСсылка,
	|	Scan_КонтрольныеТочкиLeadTimeВидыLeadTime.Ссылка.Наименование КАК КонтрольнаяТочкаНаименование,
	|	Scan_КонтрольныеТочкиLeadTimeВидыLeadTime.Ссылка.КонтрольнаяТочка КАК КонтрольнаяТочка
	|ИЗ
	...
	|
	|УПОРЯДОЧИТЬ ПО
	|	Scan_КонтрольныеТочкиLeadTimeВидыLeadTime.Порядок";
	РезультатЗапроса = Запрос.Выполнить();
	
	Выборка = РезультатЗапроса.Выбрать();
	Пока Выборка.Следующий() Цикл
		ИспользуемыеКонтрольныеТочки.Добавить(Выборка.КонтрольнаяТочкаСсылка, Выборка.КонтрольнаяТочкаНаименование, Истина);
	КонецЦикла;
	
	Если СобытиеФормыДляКонтрольныхТочек = "ОткрытиеФормы" Тогда
		СвойствоЭлементаСтруктуры = СвойствоЭлементаСтруктурыНастроек(Отчет.КомпоновщикНастроек, "ПараметрыДанных");
		ЭлементНастройки = СвойствоЭлементаСтруктуры.Элементы.Найти("КонтрольныеТочки");
		
		Если  ЭлементНастройки <> Неопределено 
			И ЭлементНастройки.Значение.Количество() > 0
			Тогда
			Для Каждого ИспользуемаяКонтрольнаяТочка Из ИспользуемыеКонтрольныеТочки Цикл
				ПараметрКонтрольнаяТочка = ЭлементНастройки.Значение.НайтиПоЗначению(ИспользуемаяКонтрольнаяТочка.Значение);
				Если ПараметрКонтрольнаяТочка = Неопределено Тогда
					ИспользуемаяКонтрольнаяТочка.Пометка = Ложь;
				Иначе
					ИспользуемаяКонтрольнаяТочка.Пометка = Истина;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
	ИначеЕсли СобытиеФормыДляКонтрольныхТочек = "ИзменениеВидаLeadTime" Тогда
		// Резерв
	КонецЕсли;
...
&НаСервере
Процедура ОбновитьЭлементыТаблицыКонтрольныхТочек()
	
	УдаляемыеРеквизиты = Новый Массив;
	УдаляемыеЭлементы  = Новый Массив;
	
	Для Каждого ЭлементКонтрольнойТочки Из Элементы.КонтрольныеТочки.ПодчиненныеЭлементы Цикл
		УдаляемыеРеквизиты.Добавить(ЭлементКонтрольнойТочки.ПутьКДанным);
		УдаляемыеЭлементы.Добавить(ЭлементКонтрольнойТочки);
	КонецЦикла;
	...
	ДобавляемыеРеквиты = Новый Массив;
	
	МассивТипов = Новый Массив;
	МассивТипов.Добавить(Тип("Булево"));
	
	РеквизитыКонтрольныхТочек = РеквизитыКонтрольныхТочек(ИспользуемыеКонтрольныеТочки);
	
	Для Каждого ИспользуемаяКонтрольнаяТочка Из ИспользуемыеКонтрольныеТочки Цикл
		РеквизитыКонтрольнойТочки = РеквизитыКонтрольныхТочек.Получить(ИспользуемаяКонтрольнаяТочка.Значение);
		
		НоваяКолонка = Новый РеквизитФормы("КонтрольнаяТочка"+РеквизитыКонтрольнойТочки.ИмяКлючевойДаты, Новый ОписаниеТипов(МассивТипов), "КонтрольныеТочки", ИспользуемаяКонтрольнаяТочка.Представление);
		ДобавляемыеРеквиты.Добавить(НоваяКолонка);
	КонецЦикла;
	
	ИзменитьРеквизиты(ДобавляемыеРеквиты);
	
	Для Каждого ИспользуемаяКонтрольнаяТочка Из ИспользуемыеКонтрольныеТочки Цикл
		РеквизитыКонтрольнойТочки = РеквизитыКонтрольныхТочек.Получить(ИспользуемаяКонтрольнаяТочка.Значение);
		
		НовыйЭлементФормы = Элементы.Добавить("КонтрольнаяТочка" + РеквизитыКонтрольнойТочки.ИмяКлючевойДаты, Тип("ПолеФормы"), Элементы.КонтрольныеТочки);
		НовыйЭлементФормы.ПутьКДанным         = "КонтрольныеТочки." + "КонтрольнаяТочка"+РеквизитыКонтрольнойТочки.ИмяКлючевойДаты;
		...
		НовыйЭлементФормы.УстановитьДействие("ПриИзменении", "КонтрольнаяТочкаПриИзменении");
	КонецЦикла;
	
	КонтрольныеТочки.Очистить();
	
	НоваяСтрока = КонтрольныеТочки.Добавить();
	Для Каждого ИспользуемаяКонтрольнаяТочка Из ИспользуемыеКонтрольныеТочки Цикл
		РеквизитыКонтрольнойТочки = РеквизитыКонтрольныхТочек.Получить(ИспользуемаяКонтрольнаяТочка.Значение);
		
		НоваяСтрока["КонтрольнаяТочка"+РеквизитыКонтрольнойТочки.ИмяКлючевойДаты] = ИспользуемаяКонтрольнаяТочка.Пометка;
	КонецЦикла;
	
КонецПроцедуры

&НаКлиенте
Процедура ВидLeadTimeПриИзменении(Элемент)
	
	СобытиеФормыДляКонтрольныхТочек = "ИзменениеВидаLeadTime";
...
Процедура КонтрольныеТочкиПриИзменении(Элемент)
	...
	Для Каждого ИспользуемаяКонтрольнаяТочка Из ИспользуемыеКонтрольныеТочки Цикл
		...
		Если "КонтрольнаяТочка"+РеквизитыКонтрольнойТочки.ИмяКлючевойДаты = Элемент.ТекущийЭлемент.Имя Тогда
			ИспользуемаяКонтрольнаяТочка.Пометка = Элемент.ТекущиеДанные[Элемент.ТекущийЭлемент.Имя];
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
&НаКлиенте
Процедура ЗаполнитьПараметрыНастроек()
	
	СвойствоЭлементаСтруктуры = СвойствоЭлементаСтруктурыНастроек(Отчет.КомпоновщикНастроек, "ПараметрыДанных");
	
	ЭлементНастройки = СвойствоЭлементаСтруктуры.Элементы.Найти("ВидLeadTime");
	ЭлементНастройки.Значение = ВидLeadTime;
	
	ЭлементНастройки = СвойствоЭлементаСтруктуры.Элементы.Найти("ВыводитьСреднееЗначениеLeadTimeПоКонтрольнымТочкам");
	ЭлементНастройки.Значение = ВыводитьСреднееЗначениеLeadTimeПоКонтрольнымТочкам;
	
	ЭлементНастройки = СвойствоЭлементаСтруктуры.Элементы.Найти("ВыводитьМедиануLeadTimeПоКонтрольнымТочкам");
	ЭлементНастройки.Значение = ВыводитьМедиануLeadTimeПоКонтрольнымТочкам;
	
	ЭлементНастройки = СвойствоЭлементаСтруктуры.Элементы.Найти("ВыводитьТолькоРазницуВДняхМеждуКонтрольнымиТочкамиLeadTime");
	ЭлементНастройки.Значение = ВыводитьТолькоРазницуВДняхМеждуКонтрольнымиТочкамиLeadTime;
	
	ЭлементНастройки = СвойствоЭлементаСтруктуры.Элементы.Найти("КонтрольныеТочки");
	ЭлементНастройки.Значение.Очистить();
	Для Каждого ИспользуемаяКонтрольнаяТочка Из ИспользуемыеКонтрольныеТочки Цикл
		Если ИспользуемаяКонтрольнаяТочка.Пометка Тогда
			ЭлементНастройки.Значение.Добавить(ИспользуемаяКонтрольнаяТочка.Значение);
		КонецЕсли;
	КонецЦикла;
	
	ЭлементНастройки = СвойствоЭлементаСтруктуры.Элементы.Найти("ВыводитьДиаграмму1");
	ЭлементНастройки.Значение = ВыводитьДиаграмму1;
	
	ЭлементНастройки = СвойствоЭлементаСтруктуры.Элементы.Найти("ВыводитьДиаграмму2");
	ЭлементНастройки.Значение = ВыводитьДиаграмму2;
	
	ЭлементНастройки = СвойствоЭлементаСтруктуры.Элементы.Найти("ВыводитьДиаграмму3");
	ЭлементНастройки.Значение = ВыводитьДиаграмму3;
	
	ЭлементНастройки = СвойствоЭлементаСтруктуры.Элементы.Найти("ВыводитьДиаграмму4");
	ЭлементНастройки.Значение = ВыводитьДиаграмму4;
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьНастройкиПараметрами(НастройкиКД)
	
	ПараметрДанных = НастройкиКД.ПараметрыДанных.Элементы.Найти("ВидLeadTime");
	Если ПараметрДанных <> Неопределено Тогда
		ВидLeadTime = ПараметрДанных.Значение;
	КонецЕсли;
	
	ПараметрДанных = НастройкиКД.ПараметрыДанных.Элементы.Найти("ВыводитьСреднееЗначениеLeadTimeПоКонтрольнымТочкам");
	Если ПараметрДанных <> Неопределено Тогда
		ВыводитьСреднееЗначениеLeadTimeПоКонтрольнымТочкам = ПараметрДанных.Значение;
	КонецЕсли;
	
	ПараметрДанных = НастройкиКД.ПараметрыДанных.Элементы.Найти("ВыводитьМедиануLeadTimeПоКонтрольнымТочкам");
	Если ПараметрДанных <> Неопределено Тогда
		ВыводитьМедиануLeadTimeПоКонтрольнымТочкам = ПараметрДанных.Значение;
	КонецЕсли;
	
	ПараметрДанных = НастройкиКД.ПараметрыДанных.Элементы.Найти("ВыводитьТолькоРазницуВДняхМеждуКонтрольнымиТочкамиLeadTime");
	Если ПараметрДанных <> Неопределено Тогда
		ВыводитьТолькоРазницуВДняхМеждуКонтрольнымиТочкамиLeadTime = ПараметрДанных.Значение;
	КонецЕсли;
	
	ПараметрДанных = НастройкиКД.ПараметрыДанных.Элементы.Найти("ВыводитьДиаграмму1");
	Если ПараметрДанных <> Неопределено Тогда
		ВыводитьДиаграмму1 = ПараметрДанных.Значение;
	КонецЕсли;
	
	ПараметрДанных = НастройкиКД.ПараметрыДанных.Элементы.Найти("ВыводитьДиаграмму2");
	Если ПараметрДанных <> Неопределено Тогда
		ВыводитьДиаграмму2 = ПараметрДанных.Значение;
	КонецЕсли;
	
	ПараметрДанных = НастройкиКД.ПараметрыДанных.Элементы.Найти("ВыводитьДиаграмму3");
	Если ПараметрДанных <> Неопределено Тогда
		ВыводитьДиаграмму3 = ПараметрДанных.Значение;
	КонецЕсли;
	
	ПараметрДанных = НастройкиКД.ПараметрыДанных.Элементы.Найти("ВыводитьДиаграмму4");
	Если ПараметрДанных <> Неопределено Тогда
		ВыводитьДиаграмму4 = ПараметрДанных.Значение;
	КонецЕсли;
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция РеквизитыКонтрольныхТочек(ИспользуемыеКонтрольныеТочки)
	
	РеквизитыКонтрольныхТочек = Новый Соответствие;
	
	Для Каждого ИспользуемаяКонтрольнаяТочка Из ИспользуемыеКонтрольныеТочки Цикл
		РеквизитыКонтрольныхТочек.Вставить(ИспользуемаяКонтрольнаяТочка.Значение, РеквизитыКонтрольнойТочки(ИспользуемаяКонтрольнаяТочка.Значение));
	КонецЦикла;
	
	Возврат РеквизитыКонтрольныхТочек;
