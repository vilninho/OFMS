...
Функция РезультатФормированияОтчета(Знач ФормированиеПриОткрытии, Непосредственно)
	...
	//rarus FominskiyAS 19.06.2019  mantis 14191 +++
	...
	Если ЭтотОбъект.КлючТекущегоВарианта = "OrderDeviation" тогда
		ЭлементВсе = Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("Все");
		ЭлементВсе.Использование = Истина;
	    ЭлементВсе.Значение = Истина;
	КонецЕсли;
		
	//ЗначениеВРеквизитФормы();
	
	Если не ПараметрыДанныхОСКД.Найти("ПоказыватьВсеЗаказыНаЗавод").Использование тогда
		//Сообщение = Новый СообщениеПользователю;
		ТекстОшибки = "Отбор ""Показывать все заказы на завод"" снимать нельзя! Небходимо выбрать ""Да"" или ""Нет""";
		//Сообщение.Сообщить();
...
Функция ПараметрыФормированияОтчета(ИмяОтчета, Непосредственно)
	...
	Если Непосредственно Тогда
		...
		//rarus kabany 27.04.2021 17684 +++
		Если ПараметрыФормированияОтчета.КлючВарианта = "DateDeviationDays" тогда
			ПараметрыФормированияОтчета.Объект.СхемаКомпоновкиДанных.НаборыДанных.НаборДанных1.Запрос = ПолучитьТекстЗапросаDateDeviationDays();	
		КонецЕсли;
		//rarus kabany 27.04.2021 17684 ---
		
	Иначе
		//rarus kabany 28.04.2021 17684 +++
		Если ПараметрыФормированияОтчета.КлючВарианта = "DateDeviationDays" тогда
			СхемаОСКД = ПолучитьИзВременногоХранилища(НастройкиОтчета.АдресСхемы); 
			СхемаОСКД.НаборыДанных.НаборДанных1.Запрос = ПолучитьТекстЗапросаDateDeviationDays();
			ПараметрыФормированияОтчета.Вставить("СхемаКД", СхемаОСКД);
		КонецЕсли;
		//rarus kabany 27.04.2021 17684 ---			
		Если НастройкиОтчета.СхемаМодифицирована Тогда						
...
Процедура УстановитьВидимостьДоступность(ПриСохраненииВарианта = Ложь)
	...
	Если Не ПриСохраненииВарианта Тогда
		...
		//Элементы.ГруппаВариантыОтчета.Видимость = ПоказыватьКомандыВыбораВариантов; //rarus bonmak 25.02.2020 14927
...
Процедура СформироватьОтчетНаСервере(ТаблДок)
	...
	Если ЭтотОбъект.КлючТекущегоВарианта = "OrderDeviation" тогда
		ЭлементВсе = ПараметрыДанныхОСКД.Найти("Все");
		ЭлементВсе.Использование = Истина;
	    ЭлементВсе.Значение = Истина;
	КонецЕсли;
		
	//rarus kabany 27.04.2021 17684 +++
	Если ЭтотОбъект.КлючТекущегоВарианта = "DateDeviationDays" тогда
		ОСКД.НаборыДанных.НаборДанных1.Запрос = ПолучитьТекстЗапросаDateDeviationDays();	
	КонецЕсли;
	//rarus kabany 27.04.2021 17684 ---
	
	Если не ПараметрыДанныхОСКД.Найти("ПоказыватьВсеЗаказыНаЗавод").Использование тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = "Отбор ""Показывать все заказы на завод"" снимать нельзя! Небходимо выбрать ""Да"" или ""Нет""";
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
	...
	МакетКомпоновки = КомпоновщикМакетаОСКД.Выполнить(ОСКД,НастройкиОСКД,ДанныеРасшифровки_);//,,Тип("ГенераторМакетаКомпоновкиДанных"),Истина); 
		
	ПроцессорКомпоновкиОСКД = Новый ПроцессорКомпоновкиДанных;
	ПроцессорКомпоновкиОСКД.Инициализировать(МакетКомпоновки,,ДанныеРасшифровки_);
	...
	АдресВоВременномХранилище = ПоместитьВоВременноеХранилище(ДанныеРасшифровки_,ЭтаФорма.УникальныйИдентификатор);
...
Процедура СформироватьОтчет(Команда)
	...
	АдресВоВременномХранилище = "ДанныеРасшифровки";
	СформироватьОтчетНаСервере(ОтчетТабличныйДокумент);
	//ДанныеРасшифровки = ПолучитьИзВременногоХранилища(АдресВоВременномХранилище);
...
Процедура ПоискПоКритериямНаСервере()
	...
	ОбъектНастройки 	= Отчет.КомпоновщикНастроек.ФиксированныеНастройки.Отбор.Элементы;
	ОбъектНастройки.Очистить();
	
	// rarus agar 02.02.2021 17091 ++
	СписокПодстрок = Новый СписокЗначений;
	СписокПодстрок.ЗагрузитьЗначения(МассивПодстрок);
	// rarus agar 02.02.2021 17091 --
	...
	Если КритерийПоиска <> Перечисления.Scan_КритерииПоискаАРМ.Дилер Тогда
		...
		НовыйОтбор.ПравоеЗначение = СписокПодстрок;
		// rarus agar 02.02.2021 17091 --
	Иначе
...
Процедура РезультатОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка, ДополнительныеПараметры)
	...
	СтандартнаяОбработка = Ложь;
	Значение = РезультатОбработкаРасшифровкиНаСервере(Расшифровка);
...
&НаСервере
Функция ДанныеРасшифровки(Расшифровка)
	ДанныеРасшифровки_ = ПолучитьИзВременногоХранилища(АдресВоВременномХранилище);
	Возврат ДанныеРасшифровки_.Элементы[Расшифровка];
КонецФункции
...
Функция РезультатОбработкаРасшифровкиНаСервере(Расшифровка)
	...
	Если ТипЗнч(Расшифровка) = Тип("ИдентификаторРасшифровкиКомпоновкиДанных") тогда
		ЭлементРасшифровки = ДанныеРасшифровки(Расшифровка);
...
//rarus kabany 27.04.2021 17684 +++
Функция ПолучитьТекстЗапросаDateDeviationDays()
...
|	И (&ПоказыватьВсеЗаказыНаЗавод
|			ИЛИ НЕ Scan_ХарактеристикиЗаказовНаЗавод.Заказ.ПометкаУдаления
...
|////////////////////////////////////////////////////////////////////////////////
|ВЫБРАТЬ
|	Scan_КлючевыеДатыПроцессов.ВидКлючевойДаты КАК ПараметрЗаказаНаЗавод,
|	НАЧАЛОПЕРИОДА(Scan_КлючевыеДатыПроцессов.Значение, ДЕНЬ) КАК ТекущееЗначение,
|	Scan_КлючевыеДатыПроцессов.Объект КАК ЗаказНаЗавод,
|	Scan_КлючевыеДатыПроцессов.Период КАК ДатаИзменения,
|	Scan_ВзаимосвязьИзделийИЗаказовСрезПоследних.Изделие КАК Изделие,
|	Scan_КлючевыеДатыПроцессов.Объект.Заказчик КАК Заказчик,
|	Scan_ХарактеристикиЗаказовНаЗаводСрезПоследних.Значение КАК Клиент
...
|		ЛЕВОЕ СОЕДИНЕНИЕ НеНужныеЗаказы КАК НеНужныеЗаказы
|		ПО Scan_КлючевыеДатыПроцессов.Объект = НеНужныеЗаказы.Заказ
|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.Scan_ХарактеристикиЗаказовНаЗавод.СрезПоследних(&ДатаК, ) КАК Scan_ХарактеристикиЗаказовНаЗаводСрезПоследних
|		ПО Scan_КлючевыеДатыПроцессов.Объект = Scan_ХарактеристикиЗаказовНаЗаводСрезПоследних.Заказ
|			И (Scan_ХарактеристикиЗаказовНаЗаводСрезПоследних.Реквизит = ЗНАЧЕНИЕ(Перечисление.Scan_ДополнительнаяИнформацияПоЗаказамНаЗавод.Клиент))
|ГДЕ
|	Scan_КлючевыеДатыПроцессов.ОбъектКлючевойДаты = &ОбъектКлючевойДаты
|	И Scan_КлючевыеДатыПроцессов.Период МЕЖДУ &ДатаН И КОНЕЦПЕРИОДА(&ДатаК, ДЕНЬ)
|	И (&ПоказыватьВсеЗаказыНаЗавод
|			ИЛИ НЕ Scan_КлючевыеДатыПроцессов.Объект.ПометкаУдаления
|				И НеНужныеЗаказы.Значение ЕСТЬ NULL)
|;
|
|////////////////////////////////////////////////////////////////////////////////
...
|			И (Scan_КлючевыеДатыПроцессов.ОбъектКлючевойДаты = &ОбъектКлючевойДаты)
...
|	НАЧАЛОПЕРИОДА(Scan_КлючевыеДатыПроцессов.Значение, ДЕНЬ) КАК ПредыдущееЗначение,
...
|	Итоги.Изделие.СОП КАК СОП,
|	Итоги.Изделие.БУ ЕСТЬ NULL
|		ИЛИ НЕ Итоги.Изделие.БУ КАК НеВыводитьБУизделия
|ИЗ
|	Итог КАК Итоги
|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
|			ИтогДневной.Изделие КАК Изделие,
|			МАКСИМУМ(ИтогДневной.ДатаИзменения) КАК ДатаИзменения
|			
|		ИЗ
|			Итог КАК ИтогДневной
|		
|		СГРУППИРОВАТЬ ПО
|			ИтогДневной.Изделие) КАК ВложенныйЗапрос
|		ПО Итоги.Изделие = ВложенныйЗапрос.Изделие
|			И Итоги.ДатаИзменения = ВложенныйЗапрос.ДатаИзменения
|ГДЕ
|	(Итоги.КлючевыеДаты = &КлючевыеДаты
|			ИЛИ &Все)
|	И (Итоги.ПредыдущееЗначение <> Итоги.ТекущееЗначение
|			ИЛИ Итоги.ПредыдущееЗначение ЕСТЬ NULL)"

// rarus kabany 23.07.2021 18035 ++
//|УПОРЯДОЧИТЬ ПО
//|	ЗаказНаЗавод,
