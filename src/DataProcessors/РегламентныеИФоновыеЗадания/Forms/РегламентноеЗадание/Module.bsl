...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		//rarus vikhle 21.06.2021 mt 17637 +++	
		Если Нрег(Лев(Задание.Метаданные.Имя,4)) = "scan" Тогда
			АдресПараметровЗадания = ПоместитьВоВременноеХранилище(Задание.Параметры, УникальныйИдентификатор);
			
			ПараметрыМетодаЗадания = Задание.Параметры;
			Если ПараметрыМетодаЗадания.Количество() > 0 Тогда
				Если ТипЗнч(ПараметрыМетодаЗадания[0]) = Тип("Структура") Тогда	
					СтруктураПараметровЗадания = Задание.Параметры[0];
					СтруктураПараметровЗадания.Свойство("ПроверятьНаЗависание", ПроверятьНаЗависание);
				КонецЕсли;	
			КонецЕсли;
		Иначе
			Элементы.ПроверятьНаЗависание.ТолькоПросмотр = Истина;
		КонецЕсли;	
		//rarus vikhle 21.06.2021 mt 17637 ---
...
Процедура ЗаписатьРегламентноеЗаданиеНаСервере()
	...
	Если Действие = "Изменить" Тогда
		//rarus vikhle mt 21.06.2021 mt 17645 +++
		Задание = РегламентныеЗадания.НайтиПоУникальномуИдентификатору(Новый УникальныйИдентификатор(Идентификатор));
		//Для нетиповых рег. заданий передаем структуру параметров
		Если Нрег(Лев(Задание.Метаданные.Имя,4)) = "scan" Тогда
			ПараметрыМетодаЗадания = ПолучитьИзВременногоХранилища(АдресПараметровЗадания);
			
			Если ПараметрыМетодаЗадания.Количество() > 0 Тогда
				Если ТипЗнч(ПараметрыМетодаЗадания[0]) = Тип("Структура") Тогда	
					ПараметрСтруктураЗадания = ПараметрыМетодаЗадания[0];
					ПараметрСтруктураЗадания.Вставить("ПроверятьНаЗависание", ПроверятьНаЗависание);	
				КонецЕсли;	
			Иначе
				ПараметрыМетодаЗадания.Добавить(Новый Структура("ПроверятьНаЗависание",ПроверятьНаЗависание));
			КонецЕсли;
			
			ПараметрыЗадания.Вставить("Параметры",ПараметрыМетодаЗадания);
		КонецЕсли;
		//rarus vikhle mt 21.06.2021 mt 17645 ---
