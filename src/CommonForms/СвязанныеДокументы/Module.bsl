...
//rarus vikhle 02.09.2020 mt 16181 +++ для открытия объектов в структуре подчиненности при вызове из начальной страницы (веб клиент) 
&НаКлиенте
Процедура ОткрытьТекущее(Команда)
	Если ЗначениеЗаполнено(Элементы.ТаблицаОтчета.ТекущаяОбласть.Расшифровка) Тогда
		ПоказатьЗначение(,Элементы.ТаблицаОтчета.ТекущаяОбласть.Расшифровка);	
	КонецЕсли;
КонецПроцедуры
...
Процедура ВывестиПредставлениеИКартинку(СтрокаДерева, Макет, ЭтоТекущийОбъект = Ложь, ЭтоПодчиненный = Неопределено)
	...
	// rarus tenkam 05.04.2019 mantis 14320 +++
	//rarus tenkam 04.07.2017 mantis 10168 +++
	Если ОбщегоНазначения.ЕстьРеквизитОбъекта("Сторнирован",СтрокаДерева.Ссылка.Метаданные()) Тогда	
		Если СтрокаДерева.Ссылка.Сторнирован Тогда
			СтрокаДерева.Представление = СтрокаДерева.Представление + " (Аннулирована)";
		КонецЕсли;
	КонецЕсли;
	//rarus tenkam 04.07.2017 mantis 10168 ---
	// rarus tenkam 05.04.2019 mantis 14320 ---
