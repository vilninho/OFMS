...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИмяПФ = Параметры.ИменаМакетов;		//rarus tenkam 01.12.2016 mantis 7750 +
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	ИначеЕсли ВРег(ИсточникВыбора.ИмяФормы) = ВРег("ОбщаяФорма.ВыборФорматаВложений")
		...
		Если ВыбранноеЗначение <> Неопределено И ВыбранноеЗначение <> КодВозвратаДиалога.Отмена Тогда
			...
			Если ТипЗнч(Параметры.ПараметрКоманды) = Тип("Массив") Тогда	//rarus tenkam 14.03.2017 TLMS 011 + 
				Если ЗначениеЗаполнено(Параметры.ПараметрКоманды[0]) Тогда
					ПараметрыОтправки.Вставить("Источник", Параметры.ПараметрКоманды[0]);
				Иначе
					ПараметрыОтправки.Вставить("Источник", ПредопределенноеЗначение("Документ.Scan_ЗаявкаНаДействие.ПустаяСсылка"));	
				КонецЕсли;
				
				//rarus tenkam 14.03.2017 TLMS 011 +++	
			ИначеЕсли ТипЗнч(Параметры.ПараметрКоманды) = Тип("ДокументСсылка.Scan_ЗаявкаПеревозчику") Тогда
				ПараметрыОтправки.Вставить("Источник", Параметры.ПараметрКоманды);
				//rarus tenkam 17.11.2017 mantis 9427 +++
			ИначеЕсли ТипЗнч(Параметры.ПараметрКоманды) = Тип("ДокументСсылка.Scan_ЧекЛистТехническогоСостоянияИзделия") Тогда
				ПараметрыОтправки.Вставить("Источник", Параметры.ПараметрКоманды);
				//rarus tenkam 17.11.2017 mantis 9427 ---
				//rarus kolg 27.11.2017 TLMS 9428 ++
			ИначеЕсли ТипЗнч(Параметры.ПараметрКоманды) = Тип("ДокументСсылка.Scan_ЗаявкаНаСервисныеРаботы") Тогда
				ПараметрыОтправки.Вставить("Источник", Параметры.ПараметрКоманды);
				//rarus kolg 27.11.2017 TLMS 9428 --
			КонецЕсли; //rarus tenkam 14.03.2017 TLMS 011 ---
			ПараметрыОтправки.Вставить("ИмяПФ", ИмяПФ);
			//rarus tenkam 01.12.2016 mantis 7750 --
...
Процедура Отправить(Команда)
	...
	Если ТипЗнч(Параметры.ПараметрКоманды) = Тип("Массив") Тогда	//rarus tenkam 14.03.2017 TLMS 011 +
		Если Параметры.ПараметрКоманды.Количество() Тогда 
			Если ТипЗнч(Параметры.ПараметрКоманды[0]) = Тип("ДокументСсылка.Scan_ЗаявкаНаДействие") Тогда
				Если ЗаявкаНеПроверена(Параметры.ПараметрКоманды[0]) Тогда
					//14191 anch 19.04.2019 +++
					//Сообщить("Нельзя отправить письмо. Завяка на действие не проверена администратором продаж!");
					 //rarus FominskiyAS 25.04.2019  mantis 14191 +++
					 //Сообщить("Нельзя отправить письмо. Заявка на действие не проверена администратором продаж!");
					 // rarus tenkam 21.08.2019 mantis 14793 +++
					 //Сообщить("ru = 'Нельзя отправить письмо. Заявка на действие не проверена администратором продаж!'; en = 'You can not send an email. Request must be checked by sales support manager!'");
					 Сообщить(НСтр("ru = 'Нельзя отправить письмо. Заявка на действие не проверена администратором продаж!'; en = 'You can not send an email. Request must be checked by sales support manager!'"));
					 // rarus tenkam 21.08.2019 mantis 14793 ---
					 //rarus FominskiyAS 25.04.2019  mantis 14191 ---  
					//14191 anch 19.04.2019 ---
					Возврат;
...
//rarus tenkam 13.12.2016 mantis 7750 ++
&НаСервере
Функция ЗаявкаНеПроверена(ЗаявкаСсылка)
	Если НЕ ЗаявкаСсылка.ПровереноАП Тогда
		Возврат Истина;
	Иначе
		Возврат Ложь;
	КонецЕсли;
