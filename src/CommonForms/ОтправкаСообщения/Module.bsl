...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// rarus agar 02.06.2021 16927 ++
	//rarus tenkam 31.08.2017 mantis 8465 +++
	...
	//КонецЕсли;
	//Если Параметры.Свойство("Копии") Тогда
	//	ПочтовыйАдресПолучателяКопии = Параметры.Копии;
	//КонецЕсли;
	//rarus tenkam 31.08.2017 mantis 8465 ---
	// rarus agar 02.06.2021 16927 --
	...
	//Если ЗначениеЗаполнено(Параметры.Источник) И ПочтовыйАдресПолучателя = "" Тогда
	//	ЗаполнитьПолеКомуПоИсточнику(Параметры.Источник, Параметры.ИмяПФ);
	//	ЗаполнитьПолеКопияПоИсточнику(Параметры.Источник, Параметры.ИмяПФ);		//rarus tenkam 19.03.2017 mantis 8303 +
	//КонецЕсли;
	//rarus tenkam 09.02.2017 mantis 8303 ---
	
	Если  ЗначениеЗаполнено(Параметры.Источник) 
		И ПочтовыеАдресаПолучателей.Количество() = 0 
		Тогда
		ПолучателиДокумента = Scan_ОтправкаПисемПоЭлектроннойПочте.ПолучитьПолучателейДокумента(Параметры.Источник, Параметры.ИмяПФ);
		Если ЗначениеЗаполнено(ПолучателиДокумента) Тогда
			НовыйПолучатель                 = ПочтовыеАдресаПолучателей.Добавить();
			НовыйПолучатель.ВариантОтправки = НСтр("ru = 'Кому:'");
			НовыйПолучатель.Представление   = СтрЗаменить(ПолучателиДокумента, Символы.ПС, " ");
		КонецЕсли;
		
		ПолучателиКопииДокумента = Scan_ОтправкаПисемПоЭлектроннойПочте.ПолучитьПолучателейКопииДокумента(Параметры.Источник, Параметры.ИмяПФ, ПолучателиДокумента);
		Если ЗначениеЗаполнено(ПолучателиКопииДокумента) Тогда
			НовыйПолучатель                 = ПочтовыеАдресаПолучателей.Добавить();
			НовыйПолучатель.ВариантОтправки = НСтр("ru = 'Копия:'");
			НовыйПолучатель.Представление   = СтрЗаменить(ПолучателиКопииДокумента, Символы.ПС, " ");
		КонецЕсли;
	КонецЕсли;
	// rarus agar 02.06.2021 16927 --
	...
	АдресОтвета = Scan_ВспомогательныеФункцииСервер.ПолучитьЭлПочтуПользователя(Пользователи.ТекущийПользователь());
	//rarus tenkam 19.03.2017 mantis 8303 ---
	// rarus agar 29.06.2021 АПК --
	
	//rarus tenkam 01.12.2016 mantis 7750 ++
	Источник = Параметры.Источник;	
	ИмяПФ = Параметры.ИмяПФ;
	//rarus tenkam 01.12.2016 mantis 7750 --
	
	//rarus tenkam 30.03.2017 mantis 9067 +++
	//rarus tenkam 15.06.2017 mantis 9633 +++
	////rarus abrant 30.05.2017 mantis 9633 +++
	//Если ЗначениеЗаполнено(Параметры.Источник) И 
	//	ЗначениеЗаполнено(Параметры.ИмяПФ) И 
	//	(Параметры.Вложения.Количество() = 1 ИЛИ 
	//	Параметры.Источник.Метаданные().Имя = "Scan_ЗаявкаНаДействие" 
	//	И Параметры.Вложения.Количество() > 1)  Тогда
	////Если ЗначениеЗаполнено(Параметры.Источник) И ЗначениеЗаполнено(Параметры.ИмяПФ) И Параметры.Вложения.Количество() = 1 Тогда
	//	//это отправка с одним вложением
	////rarus abrant 30.05.2017 mantis 9633 ---
	Если ЗначениеЗаполнено(Параметры.Источник) И ЗначениеЗаполнено(Параметры.ИмяПФ) И Параметры.Вложения.Количество() >= 1 Тогда
	//rarus tenkam 15.06.2017 mantis 9633 ---	
		ВложенныйФайл = Параметры.Вложения[0].Представление;
		ВложенныйФайл = СтрЗаменить(ВложенныйФайл, ".pdf", "");
		ВложенныйФайл = СтрЗаменить(ВложенныйФайл, ".xlsx", "");
		ВложенныйФайл = СтрЗаменить(ВложенныйФайл, ".xls", "");
		ВложенныйФайл = СтрЗаменить(ВложенныйФайл, ".ods", "");
		ВложенныйФайл = СтрЗаменить(ВложенныйФайл, ".mxl", "");
		ВложенныйФайл = СтрЗаменить(ВложенныйФайл, ".docx", "");
		ВложенныйФайл = СтрЗаменить(ВложенныйФайл, ".html", "");
		ВложенныйФайл = СтрЗаменить(ВложенныйФайл, ".txt", "");
	
		ТемаПисьма = Scan_ПраваИНастройки.Scan_Право("ТемаПисьмаПриИнтерактивнойОтправке");
		ТемаПисьма = ?(ЗначениеЗаполнено(ТемаПисьма), ТемаПисьма + " " + ВложенныйФайл,ВложенныйФайл);
		ТелоПисьма = Scan_ПраваИНастройки.Scan_Право("ТекстПисьмаПриИнтерактивнойОтправке");
		
		//rarus tenkam 31.08.2017 mantis 8465 +++		
		Если Scan_ПраваИНастройки.Scan_Право("ИспользоватьМеханизмШаблоновДляОтправкиПисем") Тогда
			//Элементы.ТелоПисьма.Вид = ВидПоляФормы.ПолеHTMLДокумента;
			
			//rarus BProg_Gladkov 04.05.2020 0015962 ++ 	
			НайденныйШаблон = Scan_ОтправкаПисемПоЭлектроннойПочте.ШаблонПоИмениПФ(ИмяПФ);
			Если ЗначениеЗаполнено(НайденныйШаблон) Тогда
				// rarus agar 02.06.2021 16927 ++
				//ПочтовыйАдресПолучателя 		= Справочники.Scan_ШаблоныПисем.ПолучитьПолучателейПисьма(НайденныйШаблон, Параметры.Источник);
				//ПочтовыйАдресПолучателяКопии 	= Справочники.Scan_ШаблоныПисем.ПолучитьКопииПисьма(НайденныйШаблон, Параметры.Источник);
				
				ПочтовыеАдресаПолучателей.Очистить();
				
				ПолучателиПисьмаПоШаблону = Справочники.Scan_ШаблоныПисем.ПолучитьПолучателейПисьма(НайденныйШаблон, Параметры.Источник);
				Если ЗначениеЗаполнено(ПолучателиПисьмаПоШаблону) Тогда
					НовыйПолучатель                 = ПочтовыеАдресаПолучателей.Добавить();
					НовыйПолучатель.ВариантОтправки = НСтр("ru = 'Кому:'");
					НовыйПолучатель.Представление   = СтрЗаменить(ПолучателиПисьмаПоШаблону, Символы.ПС, " ");
				КонецЕсли;
				
				ПолучателиКопииПисьмаПоШаблону = Справочники.Scan_ШаблоныПисем.ПолучитьКопииПисьма(НайденныйШаблон, Параметры.Источник);
				Если ЗначениеЗаполнено(ПолучателиКопииПисьмаПоШаблону) Тогда
					НовыйПолучатель                 = ПочтовыеАдресаПолучателей.Добавить();
					НовыйПолучатель.ВариантОтправки = НСтр("ru = 'Копия:'");
					НовыйПолучатель.Представление   = СтрЗаменить(ПолучателиКопииПисьмаПоШаблону, Символы.ПС, " ");
				КонецЕсли;
				
				ТемаПисьма 						= Справочники.Scan_ШаблоныПисем.ПолучитьЗаголовокПисьма(НайденныйШаблон, Параметры.Источник);
				
				ТелоПисьмаШаблона = Справочники.Scan_ШаблоныПисем.СформироватьТекст(НайденныйШаблон, Параметры.Источник);
				Если СтрНайти(ТелоПисьмаШаблона, "html") <> 0 Тогда
					ТелоПисьма.УстановитьHTML(ТелоПисьмаШаблона, ВложенияДляПисьма);
				Иначе
					ТелоПисьма.УстановитьHTML(ТекстВHTML(ТелоПисьмаШаблона), ВложенияДляПисьма);
				КонецЕсли;
				
				//ТипТекста 						= НайденныйШаблон.ТипТекста;
				// rarus agar 02.06.2021 16927 --
			Иначе
				// rarus agar 02.06.2021 16927 ++
				...
				//ТелоПисьма 					 	= "";
				ТелоПисьма.УстановитьHTML(ТекстВHTML(""), ВложенияДляПисьма);
	...
	Если ПоляЗаполненыКорректно() И ОтправитьПочтовоеСообщение(ЕстьОшибочныеПолучатели) Тогда
		...
		ПоказатьОповещениеПользователя(НСтр("ru = 'Сообщение отправлено:'"), ,
			...
			Если ТипЗнч(Источник) <> Тип("ДокументСсылка.Scan_ЗаявкаНаСервисныеРаботы") Тогда
				//rarus kolg 27.11.2017 TLMS 9428 --
				//rarus tenkam 01.12.2016 mantis 7750 ++
				Scan_ВспомогательныеФункцииСервер.ОбработатьСменуСтатусаЗаявки(Источник, ИмяПФ);
				//rarus tenkam 01.12.2016 mantis 7750 --
				//rarus kolg 27.11.2017 TLMS 9428 ++
			Иначе		
				Оповестить("УстановленНовыйВнутреннийНомер");
			КонецЕсли;
			//rarus kolg 27.11.2017 TLMS 9428 --
...
Функция ОтправитьПочтовоеСообщение(ЕстьОшибочныеПолучатели)
	...
	Если ОшибочныеПолучатели.Количество() > 0 Тогда
		...
		Если ПочтовыеАдресаПолучателей.Количество() > ОшибочныеПолучатели.Количество() Тогда
			РегистрыСведений.Scan_ОтправленныеПисьма.ЗаписатьОтправкуПисьма(ПараметрыПисьма);
			
			//rarus kolg 27.11.2017 TLMS 9428 ++
			Если ТипЗнч(ПараметрыПисьма.ДокументОтправки) = Тип("ДокументСсылка.Scan_ЗаявкаНаСервисныеРаботы") Тогда
				Scan_ДопРаботыИДоукомплектование.Scan_ПриЗаписиОтправленныеПисьма(ПараметрыПисьма.ДокументОтправки);
			КонецЕсли;	
			//rarus kolg 27.11.2017 TLMS 9428 --
		КонецЕсли;
		// rarus agar 03.06.2021 16927 --
	...
	// Rarus tenkam 12.04.2022 mantis 18433 +++
	Scan_СборСтатистики.ВыполнитьРегистрациюСтатистики("Отправка эл. писем", НайденныйШаблон.Наименование, "Отправка письма по шаблону");
	// Rarus tenkam 12.04.2022 mantis 18433 ---
...
Функция СформироватьПараметрыПисьма()
	...
	ПараметрыПисьма.Вставить("ДокументОтправки", Источник);
	ПараметрыПисьма.Вставить("Автоматически", Ложь);
	// rarus agar 29.06.2021 АПК ++
	//ПараметрыПисьма.Вставить("Пользователь", ПользователиКлиентСервер.ТекущийПользователь());
	ПараметрыПисьма.Вставить("Пользователь", Пользователи.ТекущийПользователь());
	// rarus agar 29.06.2021 АПК --
	//rarus tenkam 31.10.2017 mantis 11695 ---
	
	// rarus agar 03.06.2021 16927 ++
	ПараметрыПисьма.Вставить("Владелец", Неопределено);
	РеквизитыОтправителя = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(УчетнаяЗапись, "ИмяПользователя,АдресЭлектроннойПочты");
	ПараметрыПисьма.Вставить("РеквизитыОтправителя", РеквизитыОтправителя);
	// rarus agar 03.06.2021 16927 --
