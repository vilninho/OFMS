...
//  ОписаниеОповещенияОЗакрытии	 - Строка							 - оповещение.
//
Процедура ОткрытьФормуОтправкиПочтовогоСообщения(
	Знач Отправитель = Неопределено,
	...
	ОткрытьФорму(
		"Документ.ЭлектронноеПисьмоИсходящее.Форма.ФормаДокумента",
		ПараметрыПисьма,
		,
		,
		,
...
//
// Параметры:
//  Контакт						 - Строка, СписокЗначений, Массив	 - список получателей письма
//  Отправитель					 - СправочникСсылка.УчетныеЗаписи	 - учетная запись, с которой выполняется отправка
//  Предмет						 - Ссылка							 - предмет письма
//  Тема						 - Строка							 - тема письма
//  Текст						 - Строка							 - текст письма
...
//  ОписаниеОповещенияОЗакрытии	 - Строка							 - оповещение.
//
Процедура НаписатьЭлектронноеПисьмоКонтакту(
	Знач Контакт,
	Знач Отправитель = Неопределено,
	Знач Предмет,
	Знач Тема = "",
	...
	Адресаты = Scan_ВзаимодействияВызовСервера.ПолучитьАдресаЭлектроннойПочтыКонтакта(Контакт);
	
	Для Каждого Адресат Из Адресаты Цикл
		Адресат.Вставить("Адрес"                       , Адресат.АдресЭП);
		Адресат.Вставить("ИсточникКонтактнойИнформации", Контакт);
	КонецЦикла;
...
// Процедура - Написать сообщение SMSКонтакту
//
// Параметры:
//  Контакт				 - Строка, СписокЗначений, Массив	 - список получателей письма
//  Текст				 - Строка							 - текст письма
//  Предмет				 - Ссылка							 - предмет письма
//  ОтправлятьВТранслите - Булево							 - признак того, что сообщениери отправке должно быть преобразовано
//															   в латинские символы.
//
Процедура НаписатьСообщениеSMSКонтакту(
	Знач Контакт,
	Знач Текст = "",
	Знач Предмет = Неопределено,
	Знач ОтправлятьВТранслите = Ложь) Экспорт
	
	Телефон = Scan_ВзаимодействияВызовСервера.ПолучитьНомерТелефонаКонтакта(Контакт);
	
	Адресат = Новый Структура;
	Адресат.Вставить("Телефон"                     , Телефон);
	Адресат.Вставить("Представление"               , Телефон);
	Адресат.Вставить("ИсточникКонтактнойИнформации", Контакт);
	
	Адресаты = Новый Массив;
	Адресаты.Добавить(Адресат);
	
	ОткрытьФормуОтправкиSMS(Адресаты, Текст, Предмет, ОтправлятьВТранслите);
...
Процедура СоздатьВзаимодействиеИлиПредмет(ИмяФормыОбъекта, Основание, Источник) Экспорт
	...
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.Событие") И Источник.Элементы.Найти("СторонниеЛица") <> Неопределено
		И Источник.Элементы.СторонниеЛица.ТекущиеДанные <> Неопределено Тогда
	
	    ДанныеУчастникаИсточник = Источник.Элементы.СторонниеЛица.ТекущиеДанные;
	    ПараметрыОткрытияФормы.Вставить("СторонниеЛица",Новый Структура("Контакт,КакСвязаться,Представление",
	                                                                      ДанныеУчастникаИсточник.Контрагент,
	                                                                      ДанныеУчастникаИсточник.КонтактнаяИнформация,
	                                                                      ""));
	// -Рарус
	
	ИначеЕсли (ТипЗнч(Основание) = Тип("ДокументСсылка.СообщениеSMS") 
		// +Рарус ИСправлено ошибка БСП. условия поменяны местами
...
Процедура КомандаОбновитьИндексППДВыполнить(ИнформацияОбАктуальностиИндексаППД,ППДВключен,ИндексАктуален) Экспорт
	...
	ДополнительныеПараметры.Вставить("ИнформацияОбАктуальностиИндексаППД", ИнформацияОбАктуальностиИндексаППД);
	ДополнительныеПараметры.Вставить("ППДВключен", ППДВключен);
	ДополнительныеПараметры.Вставить("ИндексАктуален", ИндексАктуален);

	ОбработчикОповещенияОЗакрытии = Новый ОписаниеОповещения("ВопросОНеобходимостиОбновленияПолнотекстовогоПоиска",
	                                                         ЭтотОбъект,
	                                                         ДополнительныеПараметры);
	СтрВопроса = НСтр("ru = 'Обновление индекса полнотекстового поиска может занять длительное время. Продолжить?'");
	ПоказатьВопрос(ОбработчикОповещенияОЗакрытии,СтрВопроса, РежимДиалогаВопрос.ДаНет);

КонецПроцедуры

// Только для внутреннего использования. Продолжение процедуры
// КомандаОбновитьИндексППДВыполнить.
//
Процедура ВопросОНеобходимостиОбновленияПолнотекстовогоПоиска(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Результат = Scan_ВзаимодействияВызовСервера.ОбновитьИндексППД(ДополнительныеПараметры.ИнформацияОбАктуальностиИндексаППД,
	                                                         ДополнительныеПараметры.ППДВключен,
	                                                         ДополнительныеПараметры.ИндексАктуален);
	Состояние();
...
Процедура ОтработатьОповещение(Форма,ИмяСобытия, Параметр, Источник) Экспорт
	...
	Если ТипЗнч(Параметр) = Тип("Структура") И Параметр.Свойство("ТипОповещения") Тогда
		Если (Параметр.ТипОповещения = "ЗаписьВзаимодействия" ИЛИ Параметр.ТипОповещения = "ЗаписьПредмета")
			...
			Если (Форма.Предмет = Неопределено ИЛИ Scan_ВзаимодействияКлиентСервер.ЯвляетсяВзаимодействием(Форма.Предмет)) И 
		...
		ИначеЕсли Параметр.ТипОповещения = "ЗаписьКонтакта" И Параметр.Основание = Форма.Объект.Ссылка Тогда
			...
			Если ТипЗнч(Форма.Объект.Ссылка)=Тип("ДокументСсылка.Событие") Тогда
				Форма.Элементы.СторонниеЛица.ТекущиеДанные.Контрагент = Параметр.Ссылка;
	...
	ИначеЕсли ИмяСобытия = "ВыбранКонтакт" Тогда
		...
		ИначеЕсли ТипЗнч(Форма.Объект.Ссылка)=Тип("ДокументСсылка.СообщениеSMS") Тогда
			...
			Scan_ВзаимодействияКлиентСервер.ПроверитьЗаполнениеКонтактов(Форма.Объект,Форма,"СообщениеSMS");
		...
		//	Форма.Модифицированность = Истина;
		ИначеЕсли ТипЗнч(Форма.Объект.Ссылка)=Тип("ДокументСсылка.Событие") Тогда
			
			Форма.ИзменилисьКонтакты = Истина;
			
			Форма.Объект.Контрагент           = Контакт;
			Форма.Объект.КонтактнаяИнформация = Адрес;
			
			Scan_ВзаимодействияКлиентСервер.ПроверитьЗаполнениеКонтактов(Форма.Объект,Форма,"Событие");
			Форма.Модифицированность = Истина;
...
Процедура КонтактВладелецПриАктивизацииСтроки(Элемент,Форма)  Экспорт
	...
	Для каждого ЭлементМассиваОписания Из МассивОписанияКонтактов  Цикл
		Если ЭлементМассиваОписания.ИмяВладельца = ИмяТаблицыБезПрефикса Тогда
			КоллекцияОтборов = Форма["Список_" + ЭлементМассиваОписания.Имя].КомпоновщикНастроек.Настройки.Отбор;
...
Процедура ВзаимодействиеПредметПослеЗаписи(Форма,Объект,ПараметрыЗаписи,ИмяОбъектаОтправителяСообщения = "",ПосылатьОповещение = Истина) Экспорт
	...
	Если Форма.НеобходимоОповещение Тогда
		...
		Если Scan_ВзаимодействияКлиентСервер.ЯвляетсяВзаимодействием(Объект.Ссылка) Тогда
			ПараметрыЗаписи.Вставить("Предмет",Форма.Предмет);
			ПараметрыЗаписи.Вставить("ТипОповещения","ЗаписьВзаимодействия");
		ИначеЕсли Scan_ВзаимодействияКлиентСервер.ЯвляетсяПредметом(Объект.Ссылка) Тогда
