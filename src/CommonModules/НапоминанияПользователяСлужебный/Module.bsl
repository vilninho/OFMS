...
Функция ПолучитьСтандартныеРасписанияДляНапоминания() Экспорт //rarus bonmak 12.12.2019 15505 добавил Экспорт
...
Функция СоздатьНапоминание(ПараметрыНапоминания)
	...
	//Rarus Sergei 08.11.2016 7163 ++
	Если Не ЗначениеЗаполнено(Напоминание.Автор) Тогда
		Напоминание.Автор = ПользователиКлиентСервер.ТекущийПользователь();
	КонецЕсли;
	//Rarus Sergei 08.11.2016 7163 --
...
Функция СписокТекущихНапоминанийПользователя() Экспорт
	...
	|	Напоминания.ВремяСобытия КАК ВремяСобытия,
	|	Напоминания.Источник КАК Источник,
	|	Напоминания.Автор,		//rarus tenkam 25.01.2017 mantis 8172 +
	|	Напоминания.СрокНапоминания КАК СрокНапоминания,
	|	Напоминания.Описание КАК Описание,
...
// Создает новое напоминание пользователю на указанное время
Функция ОтправитьНапоминаниеПользователю(Пользователь, Текст, Время, Источник) Экспорт
	ПараметрыНапоминания = Новый Структура;
	ПараметрыНапоминания.Вставить("Описание", Текст);
	ПараметрыНапоминания.Вставить("ВремяСобытия", Время);
	ПараметрыНапоминания.Вставить("Источник", Источник);
