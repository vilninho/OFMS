...
Процедура ПроверитьСсылкиНаОбъектПослеПодтвержденияПроверки(Ответ, Параметры) Экспорт
	...
	// rarus tenkam 29.01.2018 mantis 10885 +++
	//  Ссылка на изделие есть в любом случае в продукте.	
	Если Параметры.МассивСсылок.Количество() = 1 И ТипЗнч(Параметры.МассивСсылок[0]) = Тип("СправочникСсылка.Scan_Изделия") Тогда
		//ИЛИ ТипЗнч(Параметры.МассивСсылок[0]) = Тип("СправочникСсылка.Scan_Продукты")) Тогда  // 06.02.2018 +		// rarus tenkam 14.08.2019 mantis 14427 +	
