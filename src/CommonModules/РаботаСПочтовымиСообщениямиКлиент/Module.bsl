...
Процедура СоздатьНовоеПисьмоВложенияПодготовлены(ВложенияПодготовлены, ПараметрыОтправки)
	...
	Источник = ?(ПараметрыОтправки.Свойство("Источник"),ПараметрыОтправки.Источник, Неопределено); 	
	ИмяПФ = ?(ПараметрыОтправки.Свойство("ИмяПФ"),ПараметрыОтправки.ИмяПФ, Неопределено);
	//rarus tenkam 01.12.2016 mantis 7750 --
	...
	Иначе
		ОткрытьПростуюФормуОтправкиПочтовогоСообщения(ПараметрыОтправки, ОповещениеОЗакрытииФормы,Источник, ИмяПФ);	//rarus tenkam 01.12.2016 mantis 7750 + (добавила последние 2 аргумента)
	КонецЕсли;
...
//
Процедура ОткрытьПростуюФормуОтправкиПочтовогоСообщения(ПараметрыПисьма, ОписаниеОповещенияОЗакрытии,Источник, ИмяПФ)	
	//rarus tenkam 01.12.2016 mantis 7750 ++
	ПараметрыПисьма.Вставить("Источник", Источник);
	ПараметрыПисьма.Вставить("ИмяПФ", ИмяПФ);
	//rarus tenkam 01.12.2016 mantis 7750 --
