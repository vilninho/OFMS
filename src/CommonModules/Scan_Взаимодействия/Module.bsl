...
Процедура ЗаполнитьРеквизитыПоУмолчанию(Объект, ДанныеЗаполнения) Экспорт
	...
	ЭтоВзаимодействие = Scan_ВзаимодействияКлиентСервер.ЯвляетсяВзаимодействием(Объект.Ссылка);
	...
	Если ЭтоВзаимодействие Тогда
		...
		//+Рарус Реквизит "Ответсвтенный" заменен на общий реквизит "Менеджер"
	...
	Если Scan_ВзаимодействияКлиентСервер.ЯвляетсяКонтактом(ДанныеЗаполнения)
	...
	//+РАРУС
	ИначеЕсли Scan_ВзаимодействияКлиентСервер.ЯвляетсяВзаимодействием(ДанныеЗаполнения) Тогда
	...
	ИначеЕсли Scan_ВзаимодействияКлиентСервер.ЯвляетсяПредметом(ДанныеЗаполнения) Тогда
		// На основании предмета
		//+РАРУС
		Контакты = Scan_УправлениеДиалогомСервер.ПолучитьКонтактыВзаимодействий(ДанныеЗаполнения);
		//-РАРУС
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("Структура") Тогда
		...
		Если ДанныеЗаполнения.Свойство("Предмет") И ЗначениеЗаполнено(ДанныеЗаполнения.Предмет) Тогда
			...
			Если НЕ (ДанныеЗаполнения.Свойство("Контакт") И ЗначениеЗаполнено(ДанныеЗаполнения.Контакт)) Тогда
				//+РАРУС
				Если Scan_ВзаимодействияКлиентСервер.ЯвляетсяВзаимодействием(ДанныеЗаполнения.Предмет) Тогда
					Контакты = МенеджерОбъекта.ПолучитьКонтакты(ДанныеЗаполнения.Предмет);
				Иначе
					Контакты = Scan_УправлениеДиалогомСервер.ПолучитьКонтактыВзаимодействий(ДанныеЗаполнения.Предмет);
				КонецЕсли;
				//-РАРУС
			КонецЕсли;
		КонецЕсли;
...
Процедура ПриДобавленииОбработчиковСлужебныхСобытий(КлиентскиеОбработчики, СерверныеОбработчики) Экспорт
	...
	СерверныеОбработчики["СтандартныеПодсистемы.БазоваяФункциональность\ПослеУдаленияПомеченных"].Добавить(
		"Взаимодействия");
...
Процедура ДобавитьПараметрыРаботыКлиента(Параметры) Экспорт
	...
	//+Рарус Добавлена новая функциональная опция
	Параметры.Вставить("ИспользоватьСобытия", ПолучитьФункциональнуюОпцию("ИспользоватьСобытия"));
	//-Рарус
...
Процедура ПослеУдаленияПомеченных(Знач ПараметрыВыполнения) Экспорт
	...
	Для Каждого УдаленнаяСсылка Из ПараметрыВыполнения.Удаленные Цикл
		...
		Если Scan_ВзаимодействияКлиентСервер.ЯвляетсяВзаимодействием(УдаленнаяСсылка) Тогда
...
Процедура ПриОпределенииПредставленияПредмета(СсылкаНаПредмет, Представление) Экспорт
	...
	Если ТипЗнч(СсылкаНаПредмет) = Тип("ДокументСсылка.ЭлектронноеПисьмоВходящее") ИЛИ
		...
		//+Рарус Документы убраны из взаимодейстивий
		//ТипЗнч(СсылкаНаПредмет) = Тип("ДокументСсылка.ТелефонныйЗвонок") ИЛИ 
		//ТипЗнч(СсылкаНаПредмет) = Тип("ДокументСсылка.Встреча") ИЛИ 
		//ТипЗнч(СсылкаНаПредмет) = Тип("ДокументСсылка.ЗапланированноеВзаимодействие") 
		ТипЗнч(СсылкаНаПредмет) = Тип("ДокументСсылка.Событие") Тогда
		//-Рарус
...
Процедура ПриЗаполненииВидовОграниченийПравОбъектовМетаданных(Описание) Экспорт
	...
	|Документ.Событие.Чтение.Объект.Документ.Событие
	|Документ.Событие.Изменение.Объект.Документ.Событие
	...
	//+Рарус Документы убраны из взаимодейстивий
	|ЖурналДокументов.Взаимодействия.Чтение.Объект.Документ.Событие
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПрисоединенныеФайлы") Тогда
		...
		|Справочник.СобытиеПрисоединенныеФайлы.Чтение.Объект.Документ.Событие
		|Справочник.СобытиеПрисоединенныеФайлы.Изменение.Объект.Документ.Событие
...
Процедура ПриЗаполненииТиповИсключаемыхИзВыгрузкиЗагрузки(Типы) Экспорт
	...
	//+Рарус
	Типы.Добавить(Метаданные.Константы.ИспользоватьСобытия);
	//-Рарус
	...
	Типы.Добавить(Метаданные.Справочники.СобытиеПрисоединенныеФайлы);
	...
	Типы.Добавить(Метаданные.Документы.Событие);
...
Процедура ОбновитьИнформациюОбАктуальностиИндексаППД(ИнформацияОбАктуальностиИндексаППД, ППДВключен, ИндексАктуален) Экспорт
	...
	Если ПолнотекстовыйПоиск.ПолучитьРежимПолнотекстовогоПоиска() = РежимПолнотекстовогоПоиска.Запретить Тогда
		ППДВключен = Ложь;
		ИнформацияОбАктуальностиИндексаППД = НСтр("ru = 'Полнотекстовый поиск не включен'");
	Иначе
		ППДВключен = Истина;
...
Функция ПолучитьИмяПоляДляНаименованияВладельца(ИмяТаблицы) Экспорт
	...
	МассивОписанияКонтактов = Scan_ВзаимодействияКлиентСервер.ПолучитьМассивОписанияВозможныхКонтактов();
...
Процедура УстановитьТекущимКонтакт(Контакт, Форма) Экспорт
	...
	Иначе
		...
		МассивОписанийКонтактов = Scan_ВзаимодействияКлиентСервер.ПолучитьМассивОписанияВозможныхКонтактов();
	...
	Если ЗначениеЗаполнено(Основание) И ТипЗнч(Основание) = Тип("Структура") Тогда
		Для Каждого Элемент Из Основание Цикл
			Если Элемент.Ключ = "Объект"
				И Scan_ВзаимодействияКлиентСервер.ЯвляетсяВзаимодействием(Элемент.Значение) Тогда
				Возврат Истина;
			КонецЕсли;
		КонецЦикла;
		Возврат Ложь;
...
Процедура ПодготовитьОповещения(Форма,Параметры,ИспользоватьВзаимодействиеОснование = Истина) Экспорт
	...
	Если Параметры.Свойство("Основание") И Параметры.Основание <> Неопределено Тогда
		...
		Если Scan_ВзаимодействияКлиентСервер.ЯвляетсяВзаимодействием(Параметры.Основание) Тогда
		...
		ИначеЕсли ТипЗнч(Параметры.Основание) = Тип("Структура") И
			...
			Scan_ВзаимодействияКлиентСервер.ЯвляетсяВзаимодействием(Параметры.Основание.Объект) Тогда
		...
		ИначеЕсли ТипЗнч(Параметры.Основание) = Тип("Структура") И
			...
			Scan_ВзаимодействияКлиентСервер.ЯвляетсяВзаимодействием(Параметры.Основание.Основание)) Тогда
...
Функция ПолучитьВсеКонтактыПоПредметуИлиЦепочке(Предмет, ВключатьEmail)
	...
	Если Scan_ВзаимодействияКлиентСервер.ЯвляетсяВзаимодействием(Предмет) Тогда
	...
	Иначе
		...
		ТекстЗапроса = ТекстЗапроса + 
			Scan_ВзаимодействияПереопределяемый.ПолучитьТекстЗапросаПоискКонтактовПоПредмету(Ложь, ИмяМетаданных, Истина);
	КонецЕсли;
...
Функция ПолучитьТекстЗапросаПоискКонтактовПоЦепочкеВзаимодействий(ПомещатьВоВременнуюТаблицу)
	...
	//+Рарус Добавлен документ событие
	СписокПоиска.Добавить("ЭлектронноеПисьмоВходящее",                         "ОтправительКонтакт");
	СписокПоиска.Добавить("Событие",                                           "Контрагент");
	СписокПоиска.Добавить("Событие.СторонниеЛица",                             "Контрагент");
	//-Рарус
...
Функция СтруктураРеквизитовВзаимодействия(Ссылка) Экспорт
	...
	СтруктураВозврата = Scan_ВзаимодействияКлиентСервер.ПустаяСтруктураРеквизитыВзаимодействия();
...
//	|	
	...
	|	ГДЕ
	|		ЭлектронноеПисьмоИсходящееПолучателиПисьма.Контакт В(&Контакт)) КАК ВложенныйЗапрос";
...
Функция СоздатьСписокЗначенийИнтерактивноСоздаваемыхКонтактов() Экспорт
	...
	МассивОписанияВозможныхТиповКонтактов = Scan_ВзаимодействияКлиентСервер.ПолучитьМассивОписанияВозможныхКонтактов();
...
Процедура УстановитьПредметПоДаннымЗаполнения(Параметры,Предмет) Экспорт
	...
	Если Scan_ВзаимодействияКлиентСервер.ЯвляетсяПредметом(Параметры.Основание) Тогда
		
		Предмет = Параметры.Основание;
		
	ИначеЕсли Scan_ВзаимодействияКлиентСервер.ЯвляетсяВзаимодействием(Параметры.Основание) Тогда
...
Функция ПреобразоватьАдресПоТипуИнформации(Адрес, ТипКонтактнойИнформации = Неопределено)
	...
	Если ТипКонтактнойИнформации = Перечисления.ТипыКонтактнойИнформации.Телефон Тогда
		...
		МассивСтрок = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(Адрес, ";");
		
		Для Каждого ЭлементМассива Из МассивСтрокСТелефонами Цикл
			СтруктураПолей = Scan_ВзаимодействияКлиентСервер.СтруктураПолейПоПредставлениюТелефона(ЭлементМассива);
				Если  Scan_ВзаимодействияКлиентСервер.КорректноВведенНомерТелефона(СтруктураПолей) Тогда
					МассивСтрокСТелефонами.Добавить(ЭлементМассива);
				КонецЕсли;
		КонецЦикла;
		
		Если МассивСтрокСТелефонами.Количество() > 0 Тогда
			Возврат СтроковыеФункцииКлиентСервер.ПолучитьСтрокуИзМассиваПодстрок(МассивСтрокСТелефонами, ";");
		КонецЕсли;
...
Процедура ДозаполнитьПоляКонтактов(Контакт, Представление, Адрес, ТипКонтактнойИнформации = Неопределено) Экспорт
	...
	Если ТипКонтактнойИнформации <> Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты Тогда
		...
		Если ПустаяСтрока(Адрес) Тогда
			Scan_ВзаимодействияВызовСервера.ПолучитьПредставлениеИВсюКонтактнуюИнформациюКонтакта(
	...
	Адреса = Scan_ВзаимодействияВызовСервера.ПолучитьНаименованиеИАдресаЭлектроннойПочтыКонтакта(Контакт);
...
//
Функция ПолучитьКИДляМассиваКонтактов(МассивКонтактов,Группа = "",ТипКИ) Экспорт
//Функция ПолучитьАдресаЭлектроннойПочтыДляМассиваКонтактов(МассивКонтактов,Группа = "",ТипКИ) Экспорт
//-Рарус
...
Процедура ПриЗагрузкеТипаВзаимодействийИзНастроек(Форма,Настройки) Экспорт
	...
	//+Рарус Приято решение что взаимодействия могут работать без почтнового клиента
...
Процедура ДобавитьСтраницыФормыПодбораКонтактов(Форма) Экспорт
	...
	МассивОписанийВозможныхКонтактов = Scan_ВзаимодействияКлиентСервер.ПолучитьМассивОписанияВозможныхКонтактов();
...
Процедура СоздатьОтборПоТипуСогласноФО(Список) Экспорт
	...
	ГруппаОтбора = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(
		Scan_ВзаимодействияКлиентСервер.ОтборДинамическогоСписка(Список).Элементы, "ОтборПоТипуСогласноФО",
	...
	// Так как документы взаимодействий могут работать независимо от Почтового клиента
	...
	ИначеЕсли НЕ ПолучитьФункциональнуюОпцию("ИспользоватьПрочиеВзаимодействия") Тогда
...
Процедура ИнициализироватьФормуСпискаВзаимодействий(Форма) Экспорт
	...
	//+Рарус Принято решение что взаимодействия теперь могур рабоатать без использования совместно с почтовым клиентом
	Форма.ТолькоПочта = ПолучитьФункциональнуюОпцию("ИспользоватьПочтовыйКлиент");
	Форма.ИспользуютсяПрочиеВзаимодействия = ПолучитьФункциональнуюОпцию("ИспользоватьПрочиеВзаимодействия");
	//-Рарус
	...
	//-Рарус Убрано, так как прочие взаимодействия используются без привязки к почтовому клиенту
	//Если Форма.ТолькоПочта Тогда
	...
	//	КонецЕсли;
	//КонецЕсли;
	Если НЕ Форма.ИспользуютсяПрочиеВзаимодействия И НЕ Форма.ТолькоПочта Тогда
		СоздатьОтборПоТипуСогласноФО(Форма.Список);
	КонецЕсли;
	//-Рарус
...
//	
//	//+Рарус Принято решение что Сообщение может работать без почтового клиента
...
//    "Контакты взаимодействий"
//
Функция НеЗаписыватьКонтакты(ДополнительныеСвойства) Экспорт
...
Функция ТекстЗапросаСпискаВзаимодействий(ЗначениеОтбора = Неопределено) Экспорт
	...
	|ВЫБРАТЬ
	|	ВЫБОР
	//+Рарус Документы убраны из вызаимодействий
	...
	|						КОНЕЦ
	|				КОНЕЦ
	//+Рарус ДОбавлены картинки для документа событие
	|	КОГДА ЖурналДокументовВзаимодействия.Ссылка ССЫЛКА Документ.Событие
	|		ТОГДА ВЫБОР
	|				КОГДА ЖурналДокументовВзаимодействия.ХозОперация = ЗНАЧЕНИЕ(Справочник.ХозОперации.Анкетирование)
	|					ТОГДА ВЫБОР
	|							КОГДА ЖурналДокументовВзаимодействия.ПометкаУдаления
	|								ТОГДА 2
	|							ИНАЧЕ 1
	|						КОНЕЦ
	|				КОГДА ЖурналДокументовВзаимодействия.ХозОперация = ЗНАЧЕНИЕ(Справочник.ХозОперации.ЛичнаяВстреча)
	|					ТОГДА ВЫБОР
	|							КОГДА ЖурналДокументовВзаимодействия.ПометкаУдаления
	|								ТОГДА 10
	|							ИНАЧЕ 0
	|						КОНЕЦ
	|			КОГДА ЖурналДокументовВзаимодействия.ХозОперация = ЗНАЧЕНИЕ(Справочник.ХозОперации.Событие)
	|				ТОГДА ВЫБОР
	|					КОГДА ЖурналДокументовВзаимодействия.ПометкаУдаления
	|						Тогда 12
	|					ИНАЧЕ 11
	|				КОНЕЦ
	|			КОГДА ЖурналДокументовВзаимодействия.ХозОперация = ЗНАЧЕНИЕ(Справочник.ХозОперации.ПочтовоеПисьмо)
	|				ТОГДА ВЫБОР
	|					КОГДА ЖурналДокументовВзаимодействия.ВходящееИсходящее = ЗНАЧЕНИЕ(Перечисление.ВходящееИсходящееСобытие.Входящее)
	|						ТОГДА ВЫБОР
	|							КОГДА ЖурналДокументовВзаимодействия.ПометкаУдаления
	|								ТОГДА 6
	|							ИНАЧЕ 5
	...
	|						КОНЕЦ
	|					КОНЕЦ
	|			КОГДА ЖурналДокументовВзаимодействия.ХозОперация = ЗНАЧЕНИЕ(Справочник.ХозОперации.ЭлектронноеПисьмо)
	|				ТОГДА ВЫБОР
	|					КОГДА ЖурналДокументовВзаимодействия.ВходящееИсходящее = ЗНАЧЕНИЕ(Перечисление.ВходящееИсходящееСобытие.Входящее)
	|						ТОГДА ВЫБОР
	|							КОГДА ЖурналДокументовВзаимодействия.ПометкаУдаления
	...
	|						КОНЕЦ
	|					КОНЕЦ
	|			КОГДА ЖурналДокументовВзаимодействия.ХозОперация = ЗНАЧЕНИЕ(Справочник.ХозОперации.ТелефонныйЗвонок)
	|				ТОГДА ВЫБОР
	|					КОГДА ЖурналДокументовВзаимодействия.ВходящееИсходящее = ЗНАЧЕНИЕ(Перечисление.ВходящееИсходящееСобытие.Входящее)
	|						ТОГДА ВЫБОР
	|							КОГДА ЖурналДокументовВзаимодействия.ПометкаУдаления
	...
	|						КОНЕЦ
	|					КОНЕЦ
	|			КОГДА ЖурналДокументовВзаимодействия.ХозОперация = ЗНАЧЕНИЕ(Справочник.ХозОперации.СообщениеSMS)
	|				ТОГДА ВЫБОР
	|					КОГДА ЖурналДокументовВзаимодействия.ВходящееИсходящее = ЗНАЧЕНИЕ(Перечисление.ВходящееИсходящееСобытие.Входящее)
	|						ТОГДА ВЫБОР
	|							КОГДА ЖурналДокументовВзаимодействия.ПометкаУдаления
	...
	//+Рарус Телефонный звонок удален из журнала взаимодействий
	//|	ЖурналДокументовВзаимодействия.Входящий,
	|	ЖурналДокументовВзаимодействия.ХозОперация,
	//-Рарус
	|	ЖурналДокументовВзаимодействия.Тема,
	//+Рарус Реквизит "Ответсвтенный" заменен на общий реквизит "Менеджер"
	|	ЖурналДокументовВзаимодействия.Менеджер КАК Менеджер,
	//-Рарус
	...
	ИначеЕсли Scan_ВзаимодействияКлиентСервер.ЯвляетсяПредметом(ЗначениеОтбора) ИЛИ Scan_ВзаимодействияКлиентСервер.ЯвляетсяВзаимодействием(ЗначениеОтбора) Тогда
...
Процедура ПриЗаписиДокумента(ДокументОбъект) Экспорт
	...
	ОписаниеТиповКонтактов = Новый ОписаниеТипов(Scan_ВзаимодействияКлиентСервер.ПолучитьМассивВозможныхТиповКонтактов());
	...
	Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.Событие") Тогда
		...
		НоваяСтрока = Таблица.Добавить();
		НоваяСтрока.Контакт        = ДокументОбъект.Контрагент;
	//-Рарус
...
Процедура ЗаполнитьКонтактыМассиваВзаимодействий(МассивВзаимодействий, РассчитыватьРассмотрено = Ложь) Экспорт
	...
	//|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	Событие.Ссылка КАК Взаимодействие,
	|	Событие.Контрагент КАК Контакт,
	|	"""" КАК ПредставлениеКонтакта
	|ПОМЕСТИТЬ ИнформацияОКонтактах
	|ИЗ
	|	Документ.Событие КАК Событие
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КонтактыВзаимодействий КАК КонтактыВзаимодействий
	|		ПО Событие.Ссылка = КонтактыВзаимодействий.Взаимодействие
	|ГДЕ
	|	Событие.Ссылка В
	|			(&МассивВзаимодействий)
	//-Рарус
...
Процедура ЗаполнитьПодменюСтатус(ГруппаПодменю , Форма) Экспорт
	...
	СписокСтатусов = Scan_ВзаимодействияКлиентСервер.СписокСтатусов();
...
Процедура ЗаполнитьПодменюПоТипуВзаимодействия(ГруппаПодменю , Форма) Экспорт
	...
	//+Рарус Принято решения что прочия взаимодействия могут использоваться независмо от потчового клиента
	СписокОтборов = Scan_ВзаимодействияКлиентСервер.СписокОтборовПоТипуВзаимодействий(Форма.ТолькоПочта,Форма.ИспользуютсяПрочиеВзаимодействия);
	//-Рарус
	...
	Для Инд = 0 По СписокОтборов.Количество() - 1 Цикл
		...
		ЭлементКнопкаПодменю.Заголовок = Представление;
		ЭлементКнопкаПодменю.ТолькоВоВсехДействиях = Истина;
		//+Рарус Добавляем картинку
		ЭлементКнопкаПодменю.Картинка = СписокОтборов.Получить(Инд).Картинка;
		//-Рарус
	КонецЦикла;
...
Процедура ОбработатьПодменюОтборПоТипуВзаимодействий(Форма) Экспорт
	...
	//+Рарус Принято решения что прочия взаимодействия могут использоваться независмо от потчового клиента
	ПредставлениеТипа = Scan_ВзаимодействияКлиентСервер.СписокОтборовПоТипуВзаимодействий(Форма.ТолькоПочта,Форма.ИспользуютсяПрочиеВзаимодействия).НайтиПоЗначению(Форма.ТипВзаимодействия).Представление;
	//-Рарус
