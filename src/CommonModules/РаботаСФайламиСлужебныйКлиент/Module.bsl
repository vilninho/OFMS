		...
		Если ВыбранныеФайлы.Количество() > 0  Тогда
			...
			Если ПрисоединенныеФайлыМассив.Количество() > 0 Тогда
				...
				Если ТипЗнч(ВладелецФайла) = Тип("Массив") Тогда
					Для Каждого ТекВладелецФайла Из ВладелецФайла Цикл
						ОповеститьОбИзменении(ТекВладелецФайла);
						ПараметрыОповещения = Новый Структура;
						ПараметрыОповещения.Вставить("ВладелецФайла", ТекВладелецФайла);
				...
				Иначе
					...
					ПараметрыОповещения.Вставить("ЭтоНовый"     , Истина);
					Оповестить("Запись_Файл", ПараметрыОповещения, ПрисоединенныеФайлыМассив);
				КонецЕсли;		// rarus tenkam 18.04.2019 mantis 14195 +
			КонецЕсли;
	...
	#КонецЕсли
		...
		Если ТипЗнч(ВладелецФайла) = Тип("Массив") Тогда
			Для Каждого ТекВладелецФайла Из ВладелецФайла Цикл
				...
				ПараметрыФайла.Вставить("ВладелецФайлов",              ТекВладелецФайла);
...
Процедура ПоместитьВыбранныеФайлыВХранилищеВебЗавершение(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(ВладелецФайла) = Тип("Массив") Тогда
		Для Каждого ТекВладелецФайла Из ВладелецФайла Цикл			
			ПараметрыФайла = РаботаСФайламиСлужебныйКлиентСервер.ПараметрыДобавленияФайла();
			ПараметрыФайла.ВладелецФайлов = ТекВладелецФайла; // rarus tenkam 25.06.2021 OFMS устранение ошибок Sonar Qube (АПК) было ВладелецФайла 
			ПараметрыФайла.ИмяБезРасширения = ИмяБезРасширения;
