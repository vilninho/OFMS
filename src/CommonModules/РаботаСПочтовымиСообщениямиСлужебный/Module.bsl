...
// rarus agar 27.05.2021 16927 ++
Процедура Scan_ОтправитьСообщение(Знач УчетнаяЗапись, Знач ПараметрыОтправки) Экспорт
	Перем Кому, Тема, Тело, Вложения, АдресОтвета, ТипТекста, Копии, СкрытыеКопии, ПротоколПочты, Соединение,Владелец;	//rarus tenkam mantis 7184 + (добавила владельца) 
	Перем Пользователь, Автоматически, ДокументОтправки;	//rarus tenkam 31.10.2017 mantis 11695 +
	...
	//rarus tenkam mantis 7184 ++
	Если Не ПараметрыОтправки.Свойство("Владелец", Владелец) Тогда
		Владелец = Неопределено;
	КонецЕсли;
	//rarus tenkam mantis 7184 --

	//rarus tenkam 31.10.2017 mantis 11695 +++
	Если Не ПараметрыОтправки.Свойство("Пользователь", Пользователь) Тогда
		Пользователь = ПользователиКлиентСервер.ТекущийПользователь();
	КонецЕсли;
	
	Если Не ПараметрыОтправки.Свойство("Автоматически", Автоматически) Тогда
		Автоматически = Истина;
	КонецЕсли;
	
	Если Не ПараметрыОтправки.Свойство("ДокументОтправки", ДокументОтправки) Тогда
		ДокументОтправки = Неопределено;
	КонецЕсли;
	//rarus tenkam 31.10.2017 mantis 11695 ---
	...
	Попытка
		...
		//rarus FominskiyAS 11.04.2019  mantis 14309 +++
		//НаборЗаписей = РегистрыСведений.Scan_ОтправленныеПисьма.СоздатьНаборЗаписей();
		НоваяЗапись = РегистрыСведений.Scan_ОтправленныеПисьма.СоздатьМенеджерЗаписи();
		//НаборЗаписей.Прочитать();
		//НоваяЗапись = НаборЗаписей.Добавить();
		//rarus FominskiyAS 11.04.2019  mantis 14309 --

		НоваяЗапись.ТемаПисьма = Письмо.Тема;
		НоваяЗапись.ДатаОтправки = ТекущаяДата();
		НоваяЗапись.Владелец = Владелец;	//rarus tenkam mantis 7184 +
		//rarus tenkam 31.10.2017 mantis 11695 +++
		НоваяЗапись.Пользователь = Пользователь;
		НоваяЗапись.Автоматически = Автоматически;
		НоваяЗапись.ДокументОтправки = ДокументОтправки;
		//rarus tenkam 31.10.2017 mantis 11695 ---
		
		ОтветныйАдрес="";
		Если АдресОтвета<>Неопределено Тогда
			Для каждого Адрес Из АдресОтвета Цикл
		...
		НоваяЗапись.Отправитель = РеквизитыОтправителя.АдресЭлектроннойПочты;
		НоваяЗапись.ТелоПисьма=Тело;
		ПолучателиПисьма="";
		Для каждого ЭлементКому Из Кому Цикл
		...
		ПолучателиКопии = "";
		Для Каждого ЭлементКопии Из Копии Цикл
		...
		// rarus tenkam 26.07.2018 mantis 13264 +++
		...
		Попытка
			...
			//rarus FominskiyAS 11.04.2019  mantis 14309 ---
			ЗаписьЖурналаРегистрации("Запись отправленного сообщения в регистр", УровеньЖурналаРегистрации.Примечание, Метаданные.РегистрыСведений.Scan_ОтправленныеПисьма , ,ТекстВЖР,);
		Исключение
			ЗаписьЖурналаРегистрации("Запись отправленного сообщения в регистр", УровеньЖурналаРегистрации.Ошибка, Метаданные.РегистрыСведений.Scan_ОтправленныеПисьма, ,ТекстВЖР,);
		КонецПопытки;
		// rarus tenkam 26.07.2018 mantis 13264 ---	
		
		//rarus kolg 27.11.2017 TLMS 9428 ++
		Если ТипЗнч(ДокументОтправки) = Тип("ДокументСсылка.Scan_ЗаявкаНаСервисныеРаботы") Тогда
			
			Scan_ДопРаботыИДоукомплектование.Scan_ПриЗаписиОтправленныеПисьма(ДокументОтправки);
...
Процедура ОтправитьСообщение(Знач УчетнаяЗапись, Знач ПараметрыОтправки) Экспорт
	...
	Если Не ПараметрыОтправки.Свойство("Копии", Копии) Тогда
		Копии = "";
	КонецЕсли;
	
	Если Не ПараметрыОтправки.Свойство("СкрытыеКопии", СкрытыеКопии) Тогда
		СкрытыеКопии = "";
	КонецЕсли;
	
	Если Не ПараметрыОтправки.Свойство("АдресОтвета", АдресОтвета) Тогда
		АдресОтвета = "";
	КонецЕсли;
...
Функция ЗагрузитьСообщения(Знач УчетнаяЗапись, Знач ПараметрыЗагрузки = Неопределено) Экспорт
	...
	Попытка
		Если ПолучениеЗаголовков Тогда
			...
			Если ПараметрыЗагрузки.Свойство("ПолучатьПрочитанные") Тогда
				ПараметрыОтбораIMAP = Новый Структура;
				ПараметрыОтбораIMAP.Вставить("Seen", ПараметрыЗагрузки.ПолучатьПрочитанные);
				ЗаголовкиИдентификаторы = Соединение.ПолучитьЗаголовки(ПараметрыОтбораIMAP);
				Если ЗначениеЗаполнено(ЗаголовкиИдентификаторы) тогда
